...
- fix(static-export): exported site no longer floods devtools with errors when opened directly in Chrome via file:// (v0.162.3). Three of the four error classes the user reported were real, one was harmless. **CORS-blocked images** — `cropImage.ts` set `img.crossOrigin = 'anonymous'` for any src that didn't start with `file:`. The static SPA returns relative paths like `./media/full/abc.jpg`, which don't pass the prefix check, but resolve to file:// URLs when the page itself is served from file://. Chromium then blocks the load (it has no CORS for file:// at all). Now the check uses `window.location.protocol === 'file:'` to detect the origin, not the src string. **Leaflet `Cannot read properties of undefined (reading 'clearRect')`** — `BaseMap.vue` initialised LMap with `preferCanvas: true` and immediately rendered its `<slot />`, so the first vector child (LCircleMarker / LPolyline / LGeoJson) added a path before Leaflet's canvas renderer had run `_initContainer` and assigned `_ctx`. The next-frame `_redraw → _clear` then hit `undefined.clearRect`. Now the slot is gated on a new `mapReady` flag flipped on the next tick after the LMap `ready` event, by which point `_ctx` exists. **`Uncaught (in promise) undefined` (Vue Router cancel)** — Vue Router 4 rejects superseded navigations with bare `undefined`. Static SPA's `main.ts` now installs an `unhandledrejection` filter that calls `preventDefault()` only when `e.reason === undefined` — anything with a real reason still surfaces. **"Unsafe attempt to load URL … from frame with URL …"** — Chromium quirk: every file:// URL is its own unique origin, even when navigating to the same hash. Cosmetic console warning, no functional impact, can't be silenced from app code; only fix is to serve the export over http(s).

- fix(website-export): media preview cap now explained inside the iframe instead of mislabeled as "Personregistret" (v0.162.2). The list footer in `MediaView.vue` was reading the wrong i18n key (`persons.showingOf` → "Personregistret har {total} personer") even though the view shows media. Both that footer and the gallery counter now read `media.showingOf` ("Mediaregistret har {total} filer" / "The media register contains {total} files"). When running inside the website-export preview iframe and the in-scope media count exceeds the inline-thumbnail cap (24 in `src/main/ipc/website-export.ts`), the count line swaps in a "Förhandsvisning av media begränsad till {limit} av {total} bilder" notice — the preview surfaces this via two new keys in `snapshot.settings` (`preview_media_limit`, `preview_media_total_linked`) so MediaView can read them with the existing `window.api.db.getSetting` and stay agnostic of the export pipeline. Removed the earlier misplaced "preview limited" pill from `WebsitePreview.vue`; the notice now lives where the truncated content actually is, not above the iframe.
