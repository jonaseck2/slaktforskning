...
- perf(import): GEDCOM/Holger import now batches persons, names, identifiers, media records, sources, **and pre-resolves every unique place in one pass before any event-creation phase runs.** The per-event `findOrCreatePlace` IPC roundtrip (SELECT + maybe INSERT) becomes a `Map.get()`. On a 22k-person Holger import the persons/names/identifiers/media/sources/places legs each collapse from N IPC calls to 1-2 — multi-minute wait gone.
- feat(import): determinate progress toast in the bottom-right corner. The bar moves continuously: every slow phase (places prep, media, sources, persons, families) emits `(N / M)` every 200 rows, and phase boundaries emit `(currentPhase / 13)` so the bar never sits indeterminate.
- feat(build): `npm run build` now skips the NSIS installer — produces the raw binary only. `npm run build:portable` zips up a portable distribution (`slaktforskning-portable-vX.Y.Z-<os>-<arch>.zip`) with the exe, MCP sidecar, license, and a README. Unzip and run; no installer step.
- feat(portable): the database lives next to the exe by default — drop the unzipped folder on a USB stick and the family.db travels with it. Falls back to the per-user data folder only when the exe's directory isn't writable (Program Files install path).
- fix(mcp): dev and prod MCP servers now connect on Windows — launcher reaches `npx` correctly even when the repo path has spaces, and the Node ESM boot path no longer trips on `node-sqlite3-wasm`'s named exports
- fix(build): `npm run build` now works on Windows when the repo path contains spaces, and on any host that can't cross-compile the MCP sidecar
