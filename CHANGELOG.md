...
- **feat(gedcom): full alignment with GEDCOM 5.5.1 and 7.0 — every authored field survives a per-version round-trip per the Prime Directive.** The 2026-05-19 audit (`docs/GEDCOM_AUDIT.md`) catalogued every silent loss between the DB, the importer, and the exporter; this plan closes them all. Schema gains six new tables — `notes` + `note_links` (SNOTE; shared notes attachable to any entity), `person_associations` (ASSO without event mediation; godparent / friend / colleague / enemy / neighbor / other), `name_translations` + `place_translations` (TRAN per 7.0 §LANG; alt-script + alt-language), `source_coverage_events` (SOUR/DATA/EVEN: "this source covers BIRT events from 1850-1920 in Östergötland") — plus two new columns on `events` (`is_negation`, `negation_event_type` for NO X), `persons.sex='X'` (intersex; 7.0-only), `header_metadata` JSON in `db_settings` (HEAD preservation), and the legacy `sources.repository` free-text column dropped in favour of the FK-only `source_repositories` join with importer-side structured-Repository synthesis from `_REPO_TEXT`. Exporter patches five corner cases the audit flagged: single-parent FAM silently dropped on export (orphan `parent_child` rows now emit synthetic FAMs with sex-appropriate HUSB/WIFE slot), PEDI subtype per-parent correctness (replaced `Array.find()` with per-parent collection + sort-order trick + 7.0 `3 _PARENT` disambiguator), missing FAMC/FAMS on INDI records (now emitted for both couple-FAMs and orphan-parent FAMs), same-couple-twice (multiple FAM with same HUSB+WIFE round-trip losslessly via separate `couple` rows), multi-parent triad (3+ parents emit FAM with 2 strongest + `ASSO ROLE PARENT` for extras on 7.0; lossy with disclosure on 5.5.1). Per-version round-trip evidence: GEDCOM 7.0 is lossless on every authored field that the spec carries; GEDCOM 5.5.1 is lossless or documented `lossy:5.5.1-spec-limit` per spec capability. UI surfaces every previously-buried field — Repositories CRUD (new view + panel + modal + source↔repo link section), Citations sections on Person/Place/Relationship panels, shared notes section across every entity panel + NoteModal + NotePicker, person associations section with role picker on PersonPanel, negation toggle on EventModal, translations on PersonNameModal + PlacePanel, source coverage on SourcePanel; plus modal field surfacing for `call_number`/`abstract`/`place_address`/`cause`/`date_value_end`/`name_prefix`/`name_suffix`/`name_qualifier`/`patronymic_base`/`event_participants.role`/`research_tasks.result`/`relationships.subtype`/`media.format`. Four non-GEDCOM importers (Holger, Genney, RootsMagic, Gramps) audited and mapped — Gramps gets the richest mapping (shared notes via SNOTE-shaped `<note>` + `<noteref>`, ASSO via `<personref>`, alt names + alt place names via lang attribute, source coverage via `<srcattribute>`), RootsMagic gains shared notes from `NoteTable` + witness role mapping from `WitnessTable`+`RoleTable`, Genney gets `SOURCE.NOTE` → shared notes, Holger inherits everything via the `profile='holger'` GEDCOM importer. Archive `.zip` round-trip extended to cover every new table; HTML website export inherits the new UI surfaces via the shared renderer components — `static-api.ts` extended with read-only methods matching `window.api` exactly, NO new static-only components per the design's anti-pattern guard. Round-trip fidelity is now mechanically guarded by 415 per-version tests across 11 files (`gedcom-roundtrip-corner-cases`, `gedcom-notes-roundtrip`, `gedcom-associations-roundtrip`, `gedcom-negations-roundtrip`, `gedcom-translations-roundtrip`, `gedcom-source-coverage-roundtrip`, `gedcom-sex-x-roundtrip`, `gedcom-head-preservation`, `gedcom-date-qualifiers`, `gedcom-fidelity-per-field`, `archive-roundtrip-newconcepts`). `relationships → families` rename deferred to a future plan per `docs/GEDCOM_AUDIT.md` §3 — the exporter corner-case patches close the Prime-Directive gap without it; rename is model-cleanliness work, not load-bearing. Audit: `docs/GEDCOM_AUDIT.md`. Plan archive: `2026-05-19-gedcom-alignment.md` + `-design.md`.
- **perf(website-export): preview build is roughly 4× faster on large databases.** Measured against the author's 22k-person family.db: the `previewSnapshot` stats branch dropped from ~22.8 s to ~5.8 s on first build, ~58.9 s to ~13.7 s on the second debounced rebuild. Two changes carry the win: (1) `src/api/html_site/scope.ts` replaces the per-frontier-person ancestor/descendant queries with one bulk `IN`-list query per generation (also bulk-batched the final spouse lookup) — N+1 → 1 per generation. (2) `src/api/html_site/snapshot.ts` and `src/api/html_site/preview.ts` short-circuit JS-side scope filtering when the user picks "everyone" without `excludeLiving` — every row would pass the filter anyway, so the `.filter(...)` walks across 22k persons / 37k relationships / 48k events / 70k participants / 12k media_links are skipped entirely. A `Promise.all`-parallel-query variant was tried and made things 70% **slower** (mutex contention on the single SQLite connection + starvation of concurrent quality-check IPCs) — see commit `0118f7e1` for the comment block + the revert reasoning. New opt-in dev-only timing instrumentation: `localStorage.setItem('slaktforskning-debug-preview-timing','1')` logs `[preview-timing] <phase>: NNN ms` for each preview rebuild, off by default. Plan archive: `2026-05-15-website-preview-perf.md`.
- **refactor(renderer/static): share `<AppSidebar>` and `<AppSettingsPanel>` between the Tauri app and the website-export static SPA.** Both `src/renderer/App.vue` (the Tauri shell) and `src/static/App.vue` (the static-export SPA shell) used to hand-roll their own sidebar markup and a near-identical appearance/theme/text-size/read-aloud/language settings panel — they had drifted apart (the static site shipped a bespoke `<form class="sidebar-search">` input the real app didn't have; the real app's settings rows were ahead by `menuLayout` and `addBtnStyle`). Both now mount the new shared components in `src/renderer/components/AppSettingsPanel.vue` (variant-gated: 7 rows for `renderer`, 5 for `static`) and `src/renderer/components/AppSidebar.vue` (driven by a structured `:sections` prop with a `#bottom` slot for renderer-only chrome — undo row, import-export, settings link). `src/renderer/App.vue` shrank from 1091 → 758 LOC; `src/static/App.vue` shrank from 389 → 96 LOC. The static SPA's bespoke search input is gone — replaced with a `🔍 Sök` router-link matching the renderer pattern. Renderer's horizontal-topbar mode is explicitly out of scope (static SPA never shipped horizontal layout); its settings panel adopted `<AppSettingsPanel>` so the two settings-panel sites in App.vue no longer duplicate. Structural identity is regression-tested by `tests/components/app-sidebar-shape.test.ts` (asserts both variant section sets produce the same outer chrome + `AppSettingsPanel` as a sub-component) and `tests/components/app-settings-panel.test.ts` (asserts shared rows for both variants + renderer-only gating + persistence/DOM-class side effects). The settings panel ships with self-contained scoped CSS so it survives the App.vue style-block cleanup that landed in the same waves. localStorage keys (`slaktforskning-appearance`, `slaktforskning-theme`, `textSize`, `slaktforskning-nav-orientation`, `slaktforskning-add-btn-style`) are preserved as string literals across both consumers — no settings reset for existing users. Plan archive: `2026-05-15-shared-app-sidebar.md`.
- fix: text-size selector now scales with the OS/browser font-size setting — tokens use rem instead of fixed px
- **fix(import/genney): GEDCOM-fallback path survives the sidecar's tempdir cleanup.** `importFromGenney`'s `finally` block deleted the `os.tmpdir()/genney-import-XXX` scratch directory on every return — including the early-return branch that handed the renderer a `gedcomFallbackPath` pointing **inside** that directory. The renderer's subsequent `fs_read_bytes_base64(gedcomFallbackPath)` then failed with `read: No such file or directory (os error 2)` (the `read:` prefix is project-local — `fs_read_*` in `src-tauri/src/lib.rs` wraps its `std::fs::read` error in that string — which is how diagnosis traced the failure back from the Tier 2 e2e suite). Fix: before returning the fallback path, `mkdtempSync` a sibling `genney-fallback-XXXXXX/` dir under `os.tmpdir()` (outside the doomed `tempDir`), `copyFileSync` the extracted `.ged` into it preserving the original basename, and return that stable path. Renderer cleans up the sidecar-allocated dir via `commands.fsRemoveDir(parentDir)` after consuming the GEDCOM through the existing importer path — mirrors the Holger-extracted-ged cleanup pattern. `tests/e2e/imports.spec.ts`'s `genney-gcc` case is the regression net (1 active test; `expectedPersons: 3` against `genney-small.gcc`). Plan archive: `2026-05-15-genney-e2e-path-resolution.md`.
- **feat(import): native Genney `.gcc` / `.backup` importer wired in the Tauri build.** Replaced the `notWired('Genney')` stub with a Bun-sidecar polyfill mirroring the MCP sidecar shape: `scripts/build-genney-sidecar.mjs` bundles `src/import/genney/sidecar-entry.ts` into `dist-genney/genney-import.bundle.mjs`; new `src-tauri/src/genney.rs` spawns it via tauri-plugin-shell and streams NDJSON results; new `genney_import` Tauri command. Renderer-side polyfill replays buffered progress, handles the encrypted/Derby-less fallback (loads `.ged` from the archive's tempdir through the existing GEDCOM import path), calls `holgerConsolidateMedia` to normalise media file_refs per `.claude/rules/media.md`, and fires `data:changed`. `GENNEY_LIB_DIR` env var makes the JAR cache work under the Tauri sandbox (`app_cache_dir/genney/lib`). Tiny `genney-small.gcc` fixture (263 bytes, 3 INDIs) added; `tests/e2e/imports.spec.ts`'s `genney-gcc` case is now an active test. `api.import.genneyDiscover` + the legacy `onHolgerProgress` override stay deferred with explicit `Deferred:` comments and un-defer triggers — the `notWired(label)` helper is now caller-less. Plan archive: `2026-05-14-genney-tauri-wiring.md`.
- **fix(tauri): drop `rename_all = "camelCase"` from every `#[tauri::command]` — the auto-generated Specta wrapper now actually reaches the Rust command.** Read of `tauri-macros-2.6.1/src/command/wrapper.rs:60–122` confirmed that `rename_all` renames *parameters only*, not the command name; Tauri's registry stores the literal Rust ident (`holger_extract_ged`) while the Specta-generated binding emitted `__TAURI_INVOKE("holgerExtractGed", ...)`. Every `rename_all` command was unreachable through the auto-generated binding — caught by the Tier 2 e2e suite's importer cases as `Command holgerExtractGed not found`. Swept 15 commands across `src-tauri/src/lib.rs` (12) and `src-tauri/src/media.rs` (3); renamed Rust function parameters to camelCase directly (`#[allow(non_snake_case)]`) for load-bearing commands (`holger_extract_ged`, `holger_bulk_copy_media`, `holger_consolidate_media`, `media_thumbnail`, `website_export_media`, `website_bake_preview_thumbnails`, `db_batch_run`); single-word-param commands drop `rename_all` without param renames (idempotent). F2 (PlacePanel Events save) demoted to test-bug — the panel-surface helper now clicks the first quick-segment when none is pre-selected (PlacePanel's `+ Event` doesn't pre-fill `event_type` the way PersonPanel does); no renderer behavior change. F3 (ChartView reactivity) demoted to flake/environment after clean-rebuild repro showed 5/5 green. Deliberate-red verified F1 fix is load-bearing. Result: `npm run test:e2e:full` → 170 passed / 0 failed across all 7 projects. Plan archive: `2026-05-14-post-audit-e2e-regression-triage.md`.
- **fix(scripts): `build-third-party-licenses.mjs` no longer fails noisily when `node_modules` is absent.** Subagents working in fresh worktrees (and any contributor doing a clean clone before `npm install`) used to see `npm ls --omit=dev --all exited 1 (ELSPROBLEMS)` cascade out of `build:third-party-licenses`, polluting every dispatch's report with a "this is pre-existing, unrelated" paragraph. The script now mirrors its existing "cargo license missing → warn + emit npm-only output" pattern for the new case: if `node_modules` is missing or empty, it warns once, writes a self-explanatory placeholder `THIRD_PARTY_LICENSES.txt` (placeholder names `npm install` + `npm run build:third-party-licenses` as the remediation), and exits 0. CI always runs `npm ci` before the bundle build, so shipped releases continue to produce the real 821-package list (183 npm + 638 Rust). Also drops the script's stale Electron/Chromium header line — the file now correctly describes the system-WebView model. Two new test cases in `tests/unit/scripts.thirdPartyLicenses.test.ts` exercise the empty-`node_modules` path in an isolated tmpdir. Plan archive: `2026-05-14-third-party-licenses-reliability.md`.
- **test(e2e): follow up on what the new framework caught — close all silent skips + fix the picker bug in the wild.** PersonPanel `+ Group` and `+ Task` CTAs (and MediaPanel `+ Person` / `+ Place`) silently no-op'd when their hosting section was collapsed: the picker DOM lived inside a `v-if`'d section-body, so flipping the picker's visibility ref had no observable effect. Found by the new framework; fixed by replacing the inline `showXPicker = …` handlers with `openXPicker()` helpers that auto-expand the section before opening the picker (same shape as the already-correct `openTaskPicker()`). `panel-surface.spec.ts` check 4 now covers `kind === 'picker'` sections — 10 newly-active tests across PersonPanel.Groups/Tasks, GroupPanel.People/Places/Media, ResearchTaskPanel.People/Places/Media, MediaPanel.Linked Persons/Places. Plus: `PanelDescriptor.seed` enriched to pre-populate rows for every lifecycle-parity section (PersonPanel.Groups, PersonPanel.Tasks, GroupPanel.Places/Media, ResearchTaskPanel.Places/Media, MediaPanel.Linked Places, PlacePanel.Tasks); `getSectionRowCount` recognises `.linked-row` (LinkedXSection pattern); native-binary fixture deferral codified as a `// Deferred coverage` block in `imports.spec.ts` naming the un-defer triggers. The 11 runtime-skipped lifecycle-parity tests collapsed into 0 runtime skips: sections that don't have row-shaped lifecycle (Timeline, Life map) carry a new `PanelSection.skipLifecycleParity: { canonicalOwner, reason }` flag that names the canonical sibling owning the lifecycle. Sections that don't opt out hard-assert `rowCount > 0` so a broken seed fails fast with a clear message rather than silently skipping. Result: `npx playwright test --project=panels` → 144 passed / 0 skipped (up from 126 passed / 11 skipped). Genney `notWired` re-triaged: needs a Node sidecar or Rust Derby reimplementation, deferred to a future plan. Plan archive: `2026-05-14-e2e-framework-followups.md`.
- **test(e2e): two-tier Playwright setup catches panel-CTA, reactivity, and importer regressions before they reach the running app.** `npm run test:e2e` keeps its <5 min Tier 1 contract (boot / crud / website-export / duplicates) and is now wired into CI on every PR via a new `e2e` job (`ubuntu-latest` + `xvfb-run`) — today's CI ran lint + unit only. `npm run test:e2e:full` adds three Tier 2 projects: `panels` (data-driven Surface Contract checks across all 8 right-side panels: host-flows-in, fulfills-label, lifecycle-parity, no-degradation), `reactivity` (14 triples covering every paneled view + list view + chart for `data-changed` propagation), and `imports` (6 cases round-tripping GEDCOM 5.5.1 + 7.0 + per-dialect GEDCOM exports for Holger / RootsMagic / Gramps / Family Tree Maker). Tier 2 stays local + on-demand until the repo goes public OSS with free CI minutes; the close-out evidence rule in CLAUDE.md now requires `test:e2e:full` output for UI / importer / `data-changed`-touching plans. New `SLAKTFORSKNING_HEADLESS=1` env makes the e2e binary run with off-screen window + macOS Accessory activation policy + Windows skip_taskbar — no Dock icon, no Cmd+Tab entry, no focus theft during local Tier 2 runs (60+ panel tests no longer pop 60+ windows). Six deliberate-break captures during plan close-out prove the test framework would catch each bug class: a CTA label lie (`+ Event` → `+ Bogus`), a modal opened without host context, a missing row delete affordance, a CTA short-circuiting when its section is collapsed, a consumer unsubscribing from `data-changed`, and a thrown importer all produce red tests naming the affected panel / consumer / importer. New helper files in `tests/e2e/helpers/` (`mutate-via-mcp.ts`, `seed-host-entity.ts`, `panel-descriptor.ts`) plus a hand-authored 3-person GEDCOM 7.0 fixture under `tests/e2e/fixtures/imports/`. Stale 11-project Electron-era e2e description in `.claude/rules/tests.md` replaced with the current 4+3 tier layout. Plan archive: `2026-05-13-e2e-expansion.md`.
- **refactor(mcp): sidecar migrated from `@yao-pkg/pkg` to Bun + esbuild.** The MCP sidecar runtime is now a shipped Bun 1.3.14 binary (per-platform from `src-tauri/binaries/`) spawning `dist-mcp/server.bundle.mjs` via tauri-plugin-shell. Build pipeline is one tool (`esbuild` → ESM bundle), down from two (esbuild → pkg). `scripts/build-mcp-sidecar.mjs` shrunk from ~150 LOC to 50; new `scripts/fetch-bun-binaries.mjs` (SHA-pinned `bun-binaries.lock`) handles per-platform Bun downloads, wired into `tauri.conf.json` `beforeDevCommand` / `beforeBuildCommand`. `@yao-pkg/pkg` and `@yao-pkg/pkg-fetch` removed. `kkrpc` is the JS-side typesafe RPC layer (no Rust crate — protocol-incompatible with MCP JSON-RPC 2.0); Rust spawns Bun via tauri-plugin-shell and reads/writes MCP stdio frames directly. New `tests/unit/bun-sidecar-spawn.test.ts` gate verifies `@modelcontextprotocol/sdk` runs under Bun (initialize handshake succeeds). macOS code-signing for externalBin handled by tauri-action automatically; APPLE_SIGNING_IDENTITY already wired in `release.yml`. Roadmap §1.4. Plan archive: `2026-05-14-kkrpc-sidecar.md`.
- chore: `npm run build` and `npm start` fetch the Bun sidecar binary on demand — clean checkouts no longer fail with "binaries/bun-aarch64-apple-darwin doesn't exist"
- chore: `npm install` works without `--legacy-peer-deps` — TypeScript and typescript-eslint bumped to match kkrpc's peer range
- **refactor(tauri-ipc): Specta migration — IPC layer now generates from Rust annotations.** `src/shared/channels/` (1,720 LOC, 21 files) deleted; replaced by `src/renderer/bindings.ts` auto-generated by `tauri-specta::Builder`. 47 `#[tauri::command]` functions + every signature type carry `#[specta::specta]` / `derive(specta::Type)`. Renderer's `tauri-window-api.ts` `mountWindowApi()` inlines every `domain:method` → `src/api/*` binding (mutating-wrapped for `data:changed` broadcast) and delegates Rust-backed commands to `commands.*` from `bindings.ts`. 23 channel-coverage tests deleted; `static-api-coverage.test.ts` trimmed to legacy-channel-only checks; `api-type.test-d.ts` removed. Adding a Tauri command now requires only the Rust annotation; TypeScript types regenerate per build via the `export_specta_bindings` cargo test. Adding a pure-TS handler edits one file (`tauri-window-api.ts`). `ipc-mcp-wirer` agent retired (its job is gone). Roadmap §1.3. Plan archive: `2026-05-14-specta-migration.md`.
- fix: fan chart text now fits and centres in each ring — balanced top/bottom padding gens 1-4, larger gen 5-6 rings stop name clipping, gen 7 font scaled down, gen 8 names + dates align in concentric rings
- fix: gen 8 names without dates now centre and use the full ring depth instead of being trapped in half a segment
- **perf: Rust-side baseline captures committed at `docs/baseline-perf/2026-05-14/`.** Three workloads traced via `samply`: boot (7.6s wall-clock, dyld-dominated), place-resolve (1.2s, mostly vitest startup), dedup (29.7s, with samply-own symbolication artifact called out). `summary.md` documents methodology + how-to-read. `.claude/skills/performance-profiling/SKILL.md` updated with "Baseline Storage Convention" pointer. Renderer-side traces deferred to user-driven Safari Web Inspector follow-up (Tauri on macOS = WKWebView, no Chromium DevTools). Roadmap §1.2. Plan archive: `2026-05-14-perf-baseline-capture.md`.
- **test(chart-layout): forward-looking property-assertion library** (`tests/unit/chart-layout/properties.ts`) with 7 named helpers (`assertNoOverlaps`, `assertParentDirection`, `assertGenerationAlignment`, `assertOutlineAdjacency`, `assertCoupleSpacing`, `assertConnectivity`, `assertStableExtent`). Audit's "1,663-LOC of golden snapshots" premise was wrong — re-investigation found zero `toMatchSnapshot` calls; the file was already property-style. Library ships anyway as infra; wired in alongside existing tests (123 → 130 tests); three regression fixtures verify the assertions fire with named messages. Roadmap §3.2. Plan archive: `2026-05-14-chart-layout-property-tests.md`.
- **refactor(modals): extract 14 composables for EventModal / PersonNameModal / PersonModal / RelationshipModal.** Each modal mounts dedicated form/validation/save composables (EventModal also: `useEventCitations`, `useEventParticipants`); 57/57 composable tests pass. State + validation + save logic now lives in `src/renderer/composables/use<Modal><Concern>.ts`, not at line 437 of `EventModal.vue`. Modal LOC drop: EventModal 1052→1042, PersonNameModal 701→630, PersonModal 646→521, RelationshipModal 568→504 (total −270 LOC). LOC targets from plan (≤400/≤300) were aspirational — per-modal domain orchestration (baptism companion, spouse picker, citation flow, wedding-offer, overlap-check) wasn't in the 5-composable scope; remaining extraction is a follow-up opportunity. Roadmap §3.5. Plan archive: `2026-05-14-modal-composable-extraction.md`.
- **refactor(import/gedcom): split `phases.ts` (1,163 LOC monolith) into 14 per-phase files.** Each `phase<Name>` function moves to `src/import/gedcom/phases/<kebab>.ts`; shared tag constants in `phases/shared.ts`; `phases/index.ts` is the re-export entry. Monolith deleted; `import-core.ts`'s `from './phases'` resolves through Node module resolution. Largest sub-file: `individuals.ts` at 415 LOC (colocated `KNOWN_INDI_TAGS`); all others under 200 LOC. 126/126 GEDCOM tests pass. Inter-phase context leakage via `ImportContext` (all phases share full context) explicitly preserved — narrowing is a future plan. Roadmap §3.3. Plan archive: `2026-05-14-gedcom-phases-split.md`.
- **refactor(api): split `src/api/duplicates.ts` (1,649 LOC monolith) into per-entity files.** Four dedup families (persons, places, sources, media) now live in `src/api/duplicates/<entity>.ts`; `shared.ts` holds `levenshtein` (used by places + sources + media); `index.ts` is the barrel re-export. Monolith deleted; `import from '../api/duplicates'` resolves through the directory. Per-entity normalizers (`placeNormalize`, etc.) stay private to their files — entity-specific intent, no premature unification. 134/134 dedup tests pass; no perf regression (29.77s suite). Test files repointed to leaf paths. Roadmap §3.1. Plan archive: `2026-05-14-duplicates-split.md`.
- **refactor(api): split `src/api/report_data.ts` (1,208 LOC monolith) into per-report files + shared helpers.** Seven report builders now live in `src/api/report_data/<report-name>.ts`; shared types in `types.ts`; `shared.ts` holds the helpers used by ≥2 reports (`resolveEventPlace`, `resolveEventsPlaces`, `getPrimaryName`, `getBirthSurnameForDisplay`, `findEventByType`); `index.ts` is the re-export entry. Helpers used by 1 report (or cousin-but-different patterns like alive-in-year's `pickDisplayedName`) stay private to their files per the design's "discover, not assume" rule. Monolith deleted; `import from '../api/report_data'` resolves through directory. Largest sub-file: `timeline.ts` at 473 LOC. 53/53 report-data tests pass. Roadmap §3.4. Plan archive: `2026-05-14-report-data-split.md`.
- **refactor(panels): single source of truth for entity-deletion UX (PanelDangerZone).** Six entity panels (Person, Place, Source, Media, Group, ResearchTask) each implemented an identical `panel-danger-zone` block (~50 LOC × 6). Extracted into [`src/renderer/components/PanelDangerZone.vue`](src/renderer/components/PanelDangerZone.vue) with an internal entity-type dispatcher (typed union — typo'd `entityType` is a TS error). Unified i18n: `confirmModal.deleteEntity.{action,title,body,confirm}` + `entities.<type>` in both `sv.ts` and `en.ts`. All six panels collapsed to one `<PanelDangerZone>` element each (per-panel LOC drop 13-15 lines). Verified `grep -rln 'panel-danger-zone' src/renderer/components/` returns only `PanelDangerZone.vue`; 687/687 component tests pass. **User-visible side-effect**: confirm-dialog wording is now uniform across panels (previously varied slightly per entity). Future deletion-UX changes edit one file. Roadmap §3.7. Plan archive: `2026-05-14-panel-danger-zone-extraction.md`.
- **chore(plans): audit follow-up — roadmap + 11 design/impl plan pairs + first wave of execution.** The 2026-05-14 audit (Tauri 2 + Vite 8 + awesome-tauri 2026 alignment, complexity hotspot survey, cleanup smells) translated into [`docs/plans/2026-05-14-audit-followup-roadmap.md`](docs/plans/2026-05-14-audit-followup-roadmap.md): 4 strategic plans (Vite 8 bundle regression, perf baseline capture, Specta migration, kkrpc+Bun sidecar), 7 tactical cleanups (current state verified by file inspection), 7 complexity refactors keyed to the hotspot survey, and an "already done" section for capability scoping, auto-update plugin, Electron-dep absence — all confirmed clean. Each design got a sibling implementation plan with bite-sized 2-5 min tasks. Three audit premises corrected during brainstorming: 3.5 modal bottleneck is `<script setup>` not `<template>` (composable extraction, not field sub-components); 3.7 premise was already shipped (`usePanelSections` exists, 8/10 panels use it) — reframed around the real duplication, panel-danger-zone blocks in six entity panels; 3.2 target is the 1,663-LOC golden-snapshot test, not `hourglass.ts` (Phase 1 = property assertions; Phase 2 hourglass refactor deferred to follow-up stub). First execution wave dispatched in parallel via worktree-isolated subagents: 3.3 Tasks 1-2 landed (GEDCOM `phases/shared.ts` tag-constants extracted; `gedcom.test.ts` 126/126); 3.5 Tasks 1-2 landed (`useEventForm` composable + 3-test vitest spec); 3.7 Tasks 1-2 landed (`PanelDangerZone` component + 5-test vitest spec + `confirmModal.deleteEntity.*` i18n keys in `sv.ts` and `en.ts`). Plan 1.2 perf baseline subagent returned BLOCKED with useful diagnostics — Tauri on macOS uses WKWebView, not Chromium DevTools; the plan needs a Safari-Web-Inspector or Rust-first (samply) revision before re-dispatch. 1.1 Vite 8 baseline still in-flight; 1.3 / 1.4 / 3.1 / 3.4 / next rounds of 3.3 / 3.5 / 3.7 pending.
- **docs(plans): archive the Tauri-port RCA + post-Tauri audit synthesis.** Both 2026-05-12 documents shipped their value: the RCA's seven follow-up plans are all archived (`skipped-tests-cleanup`, `gazetteer-lazy-chunks`, `e2e-test-repair`, crate-rename shipped in 0.254.0, `vite-7-upgrade` + follow-up `vite-8-upgrade`, `app-a11y-gaps`, `bulk-api-functions`); L1–L7 lessons all landed in `.claude/rules/plans.md` + `CLAUDE.md` (L7 — "e2e is load-bearing verification; running it is part of every close-out" — added in this commit alongside the archive); gap #16 (release.yml stale-reference audit) verified clean (triggers on `main`, generic cache keys, zero retired-name hits, repo URL matches remote, current Tauri-action / Node 22 / webkit2gtk-4.1 / OS matrix). Residual work (memory hygiene + any new code-health items) lives in the 2026-05-14 audit follow-up roadmap. Plan archives: `2026-05-12-tauri-port-rca.md`, `2026-05-12-audit-recommendation.md`.
- **perf(import): finish bulk-batching of the RootsMagic importer's api/-layer hot loops.** `src/import/rootsmagic/transform.ts` was the last importer still calling singular `createPerson` / `addPersonName` / `createEvent` / `createCitation` / etc. per row inside phase loops; this commit collapses all 13 such callsites into the same collect+flush shape `phases.ts` adopted earlier — UUIDs pre-allocated in JS via `crypto.randomUUID()` and per-phase row buffers flushed via the existing `bulk*` api/ functions in FK topo order (sources → persons → identifiers → names → couple_relationships → parent_child_relationships → events → event_participants → citations → media → media_links). `findOrCreatePlace` stays singular per the plan's deviations (place creation is interleaved with lookup; places are bounded). Plan archive: `2026-05-12-bulk-api-functions.md`. Also resolves the 0.257.3 / 0.257.4 manifest drift the 2026-05-12 audit flagged — `Cargo.toml`, `Cargo.lock`, and `tauri.conf.json` are now back in sync with `package.json` at 0.257.5.
- **chore(toolchain): lift Vite 7.3.3 → 8.0.12 and bump the MCP sidecar's bundled Node from 20 → 22.** Vite 8 swaps the underlying build pipeline from Rollup/esbuild to Rolldown/Oxc with a compatibility layer; renderer + static SPA builds reproduce the same chunk shape (71 separate gazetteer JSON assets + 1 inlined under the default `assetsInlineLimit: 4096` for the 2.4 KB `mc-quartiers.json`, largest chunk 5.7 MB — no eager-collapse) but in 839 ms instead of multi-second Rollup runs. `vite.renderer.config.ts`'s `minify: 'esbuild'` is now a deprecated alias that auto-routes through the Oxc Minifier (no edit needed). MCP sidecar pkg targets bumped `node20-*` → `node22-*` in `scripts/build-mcp-sidecar.mjs` and esbuild bundle target `--target=node20` → `--target=node22`; release.yml comment updated to match. `@yao-pkg/pkg-fetch` ships prebuilt `node22-{macos-arm64,macos-x64,win-x64,linux-x64}` binaries, no new dep. Companion peer-dep alignment (no version change): `@vitejs/plugin-vue` 6.0.6, `vite-plugin-node-polyfills` 0.26.0, `vite-plugin-singlefile` 2.3.3, `vitest` 4.1.6 already declared `vite ^8`. CI hosts (`ci.yml`, `release.yml`) already on Node 22, which satisfies Vite 8's `^20.19.0 || >=22.12.0` engine floor. Plan archive: `2026-05-13-vite-8-upgrade.md`.
- ci: bumping the version + pushing to main now auto-publishes installers (.dmg, .exe, .AppImage) to GitHub Releases
- feat(macOS): About Släktforskning menu opens the in-app About modal with version, repo link, and licenses
- chore: `npm run build` produces native installers (.dmg/.exe/.AppImage) directly — no more zip wrapper
...
- fix: import report stays open until closed; large Holger imports no longer fail on SQL variable limit
- fix: Windows MCP connects and npm run build works with spaces in repo path
- **perf(import): bulk-insert batching collapses N per-row IPC roundtrips into a single `db_batch_run` call across the importer hot loops, killing the multi-hour wait the post-Tauri-port build introduced for large imports.** New Rust command `db_batch_run` (`src-tauri/src/db.rs` + `lib.rs`) takes one prepared SQL string + many parameter rows, holds the connection mutex for the whole batch, prepares the statement once, and iterates rows under the lock — mid-batch failures propagate so the surrounding `BEGIN/COMMIT` ROLLBACKs the whole batch. Renderer-side `Statement.runBatch` (Tauri shim) plus `runBatch` / `runBatchOnStatement` helpers in `src/api/db.ts` give backend-agnostic Promise<RunResult[]> shape — Electron/node-sqlite3-wasm falls back to a sync per-row loop where the IPC cost being avoided doesn't exist. Migrated importer hot paths: Genney transform's 16 `stmts.X.run([...])` callsites collapse to per-section `flushAll()` (16k per-row IPC calls → 9 batched calls on a 1000-person fixture); gramps' `parent_place_id` UPDATE loop becomes one `runBatch`; `consolidateMediaFolder`'s 12k per-row `UPDATE media SET file_ref = ?` rewrites batch per-worker (8-worker pool × per-1000-row flushes). New regression test `tests/unit/import-batching.test.ts` mocks `@tauri-apps/api/core`'s `invoke` to route through a real in-process node-sqlite3-wasm DB while counting calls; asserts wall-clock < 8 s and bulk-IPC call count stays in the small-constant range (not row-proportional). `.claude/rules/api.md` "SQLite bulk-write performance" grew the mandatory "Use runBatch" clause. **GEDCOM phases (Holger import) and RootsMagic transform deferred** — they call api/ functions (`createPerson`, `addPersonName`, `createEvent`, `createCitation`) per-row, so a clean batch migration requires bulk variants of the api/ CRUD layer; that's its own follow-up plan. The dominant late-stage cost on a real 1.5 GB Holger import (consolidateMediaFolder) is delivered by this plan. Plan archive: `2026-05-12-bulk-insert-batching.md`.
...
- chore: commit + release skills now bump the Tauri-side version manifests alongside `package.json` so the build banner can't drift again
- **chore(rename): retire the proof-of-concept `tauri-spike` crate / identifier name; the app now identifies as `slaktforskning` end-to-end.** `src-tauri/Cargo.toml`'s `[package].name` and `[lib].name` are `slaktforskning` / `slaktforskning_lib`; `src-tauri/tauri.conf.json`'s bundle `identifier` is `com.slaktforskning.app`; the inner Mach-O binary, the e2e fixture's binary-lookup, the MCP probe handshake string, and the `mcp-tauri.mjs` fallback DB path all follow. The fallback path is now read from `tauri.conf.json` so the next rename is a one-place edit. **Heads-up for anyone with legacy dev data** under the old identifier path (the app has never shipped to a public user, so this only affects local dev databases): on macOS, `mv ~/Library/Application Support/com.slaktforskning.tauri-spike/* ~/Library/Application Support/com.slaktforskning.app/`; on Linux, swap `~/.local/share/com.slaktforskning.tauri-spike/` → `~/.local/share/com.slaktforskning.app/`; on Windows, swap `%APPDATA%\com.slaktforskning.tauri-spike\` → `%APPDATA%\com.slaktforskning.app\`. Plan archive: `2026-05-12-rename-tauri-spike.md`.
- **chore(toolchain): lift Vite 5.4.21 → 7.3.3 (the Tauri 2.x default).** Every `npm run build` and `vite build` log no longer opens with "The CJS build of Vite's Node API is deprecated" — Vite 7 ships ESM-only. `package.json` adds `"type": "module"` so node loads `.js`/`.mjs` configs as ESM by default; `vite.static.config.ts` derives `__dirname` via `fileURLToPath(import.meta.url)` to keep `outDir` resolving from the config's own directory under module semantics. Companion bumps: `@vitejs/plugin-vue` 5.2.4 → 6.0.6, `vitest` + `@vitest/coverage-v8` 4.1.0/4.1.2 → 4.1.6 (Vitest 4 supports Vite 7). `vite-plugin-node-polyfills` (0.26.0), `vite-plugin-singlefile` (2.3.3), and `rollup-plugin-visualizer` (7.0.1) already supported Vite 7 — no version change. No user-facing behavior change: renderer bundle output, dev server, and `dist-static/index.html` shape unchanged. Plan archive: `2026-05-12-vite-7-upgrade.md`.
- **test(e2e): repair the Playwright suite (boot + duplicates timeouts); rename `[smoke]` project to `[boot]` per the `.claude/rules/plans.md` smoke-antipattern rule; wire `npx playwright test` into the close-out evidence template.** Two of four projects had been failing on every run since the Tauri port — `[smoke]` because `app.test.ts` was still hitting the Electron-era `/execute_js` endpoint (Tauri serves `/eval` and returns the raw value, not `{ result }`), and `[duplicates]` because `mergeMedia` was never wired into the Tauri build (the channel registry intentionally skips it on the Electron side because `mergeMedia` does sync `fs` work that's banned in worker handlers; the Tauri build has no worker thread but had no polyfill either, so `window.api.duplicates.mergeMedia` was undefined). Both repaired: the boot test now uses `/eval` with the Tauri response shape, and a polyfill in `src/renderer/tauri-window-api.ts` exposes `duplicates.mergeMedia` (computes `dbPath` from `db_current_path` and calls `api/duplicates.ts` directly). The Playwright `[smoke]` project is renamed to `[boot]` and every comment / `startApp(..., 'smoke')` tag in `app.test.ts` and `duplicates.spec.ts` follows; `grep -rn smoke tests/e2e playwright.config.ts` now returns zero. CLAUDE.md "Finishing a plan" step 0 grew an explicit `npx playwright test → 4 passed (Xs)` evidence line item alongside `npm test` and `npm run build`. New allowlist entry in `tests/unit/tauri-channel-coverage.test.ts` records `duplicates:mergeMedia` as a known main-only Electron channel polyfilled by Tauri. Plan archive: `2026-05-12-e2e-test-repair.md`.
- **fix(a11y): close the screen-reader gaps the new `ui_aria_audit` MCP tool surfaced in Settings → Länkregler.** The Settings tab strip (Databas / Standardval / Länkregler / Ortsregister), the chart-kind tab strip on `/persons` (Stamtavla / Timglas / Efterkommande / Solfjäder / Tidslinje), and the Import/Export tab strip now expose the ARIA tab pattern — `role="tablist"` on the bar, `role="tab"` + `aria-selected` + `aria-controls` per chip, roving `tabindex`, and `role="tabpanel"` + `aria-labelledby` on the content. NVDA / VoiceOver now announce "Länkregler, tab, 3 of 4 selected" instead of "Länkregler, button, 3 of 11". `<main>` carries an `aria-label` derived from the active route's i18n title so `D` (NVDA) / `VO+U` (VoiceOver) can jump to it by name. Modal form inputs across CitationModal, EventModal, GroupModal, LinkRuleModal, PersonModal, PersonNameModal, PlaceModal, RelationshipModal, ResearchTaskModal, SourceModal now have `<label for=>` + `id=` associations (programmatic, not just visual). The Aktiva regler row-checkboxes carry per-row `aria-label` (e.g. `"Aktiverad: Riksarkivet"`) so screen readers say what each toggle controls. PlacePicker / PersonPicker inputs gained `aria-label` so the inner combobox is named without a host-modal label. The MCP audit resolver also gained spec-correct support for wrapping `<label>` ancestors as programmatic associations. New CI test `tests/components/aria-audit-zero-findings.test.ts` mounts each covered surface in happy-dom and runs the same audit logic the dev MCP runs against the live app, asserting findings is either empty or a per-surface allow-list (which starts empty); any new gap fails CI. New unit test `tests/components/FilterChips-tablist.test.ts` pins the tablist contract (role + aria-selected + aria-controls + roving tabindex + filter-mode opt-out). Plan archive: `2026-05-12-app-a11y-gaps.md`.
- **test(reporting): convert 112 registry-excluded skips to passing assertions; the suite summary's `skipped` count is now a meaningful signal.** `tests/unit/gedcom-fidelity-per-field.test.ts` previously emitted one `it.skip(...)` per `(table, column, gedcom-version)` triple where the column was documented in `gedcom_fidelity_registry.ts` as `excluded` (audit timestamps, UUIDs, derived `normalized_name`, etc.). The skip-with-reason convention surfaced "documented as not round-trippable, here's why" in test names but produced 112 noise lines in every test summary, masking the one signal that count is supposed to carry: *is anyone hiding a broken test with `it.skip()` to make CI green?* Per-cell coverage of excluded columns is preserved — each cell still runs a passing test with the registry's reason in the test name, asserted via `expect(...)`. New standing-guard test `tests/unit/no-skipped-tests.test.ts` statically scans `tests/**/*.ts` for `it.skip` / `test.skip` / `describe.skip` / `it.todo` / `test.todo` antipatterns and fails CI if any return; conditional `.skipIf(...)` / `.runIf(...)` (legitimate fixture-presence guards) are explicitly allowed. Plan archive: `2026-05-12-skipped-tests-cleanup.md`.
- **perf(gazetteers): bundled gazetteers now ship as 72 lazy-loaded JSON assets — cold-start memory drops, production build no longer needs an 8 GB Node heap.** `src/renderer/empty-gazetteers.ts` switched its `import.meta.glob` from `{ eager: true, import: 'default' }` (which inlined every JSON into one ~30 MB chunk and OOM'd Vite's rollup pass on the default 2 GB heap) to `{ query: '?url' }` — every JSON ships under `dist-tauri/assets/<id>-<hash>.json` and the webview fetches + parses each one on demand. The previously-30 MB `tauri-window-api-*.js` chunk is now 504 KB. The `cross-env NODE_OPTIONS=--max-old-space-size=8192` workaround is removed from `start` / `dev` / `build` / `build:e2e` in `package.json`; `cross-env` itself is dropped from `devDependencies`. The renderer-side gazetteer surface is now async (`getAllGazetteers` / `getGazetteerById` return `Promise<…>`); the Node-side `bundled.ts` (used by tests + the MCP sidecar) stays synchronous. New regression test `tests/unit/empty-gazetteers-no-eager.test.ts` asserts the glob declares `query: '?url'` so nobody silently re-introduces JSON-as-module loading and the heap-bump workaround. Plan archive: `2026-05-12-gazetteer-lazy-chunks.md`.
- **chore(electron-retire): drop the Electron entry points and rename `npm start` back to launching the app.** The Tauri merge in 0.252.0 kept `src/main/**`, `src/preload/**`, `forge.config.ts`, `vite.{main,preload,worker}.config.ts`, and the `electron-forge` deps as a cross-over safety net. They ship to neither Tauri nor Electron after that release, so this commit removes them: `npm start` is now `tauri dev`, `npm run build` is `tauri build`, and `npm run make` / `npm run package` / `npm run publish` are gone (run `npm run build` instead). Runtime-neutral helpers move to `src/shared/` — `db-worker-state.ts` (DI state accessors used by the channel registry), `db-worker-broadcast.ts` (DI broadcast helper), and `preview-html-inject.ts` (the pure HTML-swap used by `tauri-window-api.ts` for the website preview iframe). The duplicate `src-tauri/examples/walfix.rs` is dropped — `scripts/walfix.mjs` (using Node 22.5+'s native sqlite) is the canonical WAL-recovery tool. GitHub Actions: `ci.yml` and `release.yml` (Electron) deleted; the previously-suffixed `tauri-ci.yml` / `release-tauri.yml` are renamed to take those names. Total: ~5000 lines of dead code removed; `node_modules/` no longer has any `electron`, `@electron-forge`, `@electron/fuses`, or `electron-squirrel-startup` entries.
- **feat(dev-mcp): seven ARIA tools that mirror what a screen-reader user experiences.** `ui_aria_list` and `ui_aria_invoke` (drive the app by accessible name with ambiguity-with-candidates errors); `ui_aria_tab_order` (sequential focus walk — what pressing Tab repeatedly would visit); `ui_aria_landmarks` (every landmark with `has_name` flag); `ui_aria_headings` (every `<h1>`–`<h6>` and `[role="heading"][aria-level]`); `ui_aria_read` (the prose-flow stream — headings + paragraphs + list items + interactables in DOM order with state); `ui_aria_audit` (six finding-kinds with severity ranking and per-kind hint strings — `unnamed_interactable`, `unnamed_landmark`, `input_without_label`, `tab_strip_without_role`, `positive_tabindex`, `disabled_focusable`). Element state (`pressed` / `expanded` / `selected` / `checked` incl. `'mixed'` / `current` / `busy` / `invalid` / `required`) surfaces on every result. Accessible-name resolution honors a 7-step priority — `v-narrate` (the screen-reader mode's curated text) wins over `aria-label` over `aria-labelledby` over `<label for>` over `textContent` over `placeholder` over `title`. Region resolution is conservative — only landmarks with an *explicit* accessible name count, so bare `<main>` / `<header>` / `<aside>` without `aria-label` resolve to `region: null` instead of flooding it with the landmark's descendant text. The live smoke against the running app surfaced 27 real a11y gaps in Settings → Länkregler in a single call (Settings chip strip lacking `role="tab"`, modal inputs without label associations, 12 unnamed row-checkboxes, unnamed `<main>`) — tracked separately in `docs/plans/2026-05-12-app-a11y-gaps.md`. Renderer-side change: `src/renderer/directives/narrate.ts` exposes its `narrationMap` WeakMap on `window.__narrationMap` for the dev MCP to read; GC-safe (entries disappear when Vue unmounts the element). New unit-test class: serialization round-trip — every `buildAria*Script` output is evaluated in the test environment and compared against calling the resolver directly, catching the drift class that bit v1 twice (esbuild's `__name` helper undefined in the renderer; bare-`<main>` region flooding). Plan archive: `2026-05-11-aria-mcp-tools.md`.
- **feat(tauri-port): the app now ships as a Tauri 2.x binary, not Electron.** The Vue 3 renderer is unchanged from the user's perspective, but it now runs inside a native webview backed by a Rust core (`src-tauri/`). SQLite is rusqlite, IPC is Tauri commands routed through the same channel-registry walked at boot in `src/renderer/tauri-window-api.ts`, the MCP server runs as a sidecar process (no `npx` on PATH required), and auto-update is the Tauri 2.x updater plugin. Installer size dropped from ~280 MB to a fraction of that; idle and loaded RAM dropped substantially. The Electron entry points (`src/main/**`, `src/preload/**`, db-worker, electron-forge config) are retained in source for the duration of the cross-over window — they ship to neither Tauri nor Electron after this release. Goal-anchor verification: the Holger reference user round-tripped a real OurKind GEDCOM through the Tauri UI; Windows + macOS smoked. Archive of `tauri-port-evaluation` (spike → "go" verdict) + `2026-05-10-tauri-full-port` (the port itself) + `2026-05-10-tauri-port-completion-{audit,plan}` (gap close) + `2026-05-10-tauri-test-migration` (test pyramid migration).
- fix(tauri): `getDbSetting` / `setDbSetting` / `deleteDbSetting` now `await` `stmt.get()` / `stmt.run()`. Without the awaits, the Tauri DB shim's Promise-returning Statement methods silently degraded every per-DB setting read to `null` (`researcher_*`, `default_person_id`, `gazetteer_config`, `link_rules_config`, `event_defaults_config`) and made the writes fire-and-forget. Effect downstream: reports' SUBM block missed researcher info, GEDCOM export's submitter record was empty, the tree-subject auto-open silently no-op'd, the gazetteer scope reset to the bundled default on every reload, custom link rules vanished from the matcher.
- fix(onboarding): the coachmark bubble no longer renders in the top-left of the screen before its anchor lays out — `Coachmark.vue` now reads the anchor's `getBoundingClientRect()` and stays `visibility: hidden` until the anchor reports a non-zero rect.
- fix(person-panel): the Identifiers section is hidden when the person has zero identifiers — previously a permanently-empty section sat under every name with no data. The child component stays mounted via `v-show` so its live count drives re-appearance the moment a first identifier lands.
- feat(nav): Search is now in the sidebar (vertical) and topbar (horizontal) — `/search` was reachable only by typed URL.
- feat(skills): new `genealogy` skill captures the order-of-operations for end-to-end research sessions (pre-flight DB-divergence check, custom gazetteer authoring contract, GEDCOM round-trip verification, photo-URL pitfalls, eight friction-log entries from a real Bernadotte test) — pairs with `slaktforskning-mcp` (tool list) and `slaktforskning-mcp-dev` (extending the tools).
