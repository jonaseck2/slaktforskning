...
- feat(persons): unify "selected person" and "focal person" into a single concept. The dual `selectedPersonId` / `personId` distinction in `PersonsView` is removed — the URL `/persons/:id` is the single source of truth for the current person. Selecting a person (in tree, list, sidebar, direct URL, or chart arrow-key nav) makes them the chart focal, opens their panel, and updates the sidebar's "Vald person" indicator. The `selectNode`/`showInTree` helpers and the "Fokusera" panel-header button are removed — `navigateTo` is the single click handler. Outline placeholders in the chart now anchor to the focal (since selected = focal). Sidebar label `nav.focusPerson` rephrased to "Vald person" / "Selected person". `PersonPanel`'s `showTreeBtn` prop and `'show-in-tree'` event removed.
