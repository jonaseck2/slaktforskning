...
- **refactor(api): split `src/api/report_data.ts` (1,208 LOC monolith) into per-report files.** Seven report builders (`getPersonSummary`, `getFamilyUnit`, `getAncestorTree`, `getPlaceHistory`, `getResearchGaps`, `getTimeline`, `getAliveInYear`) now live in `src/api/report_data/<report-name>.ts`; shared types in `types.ts`; `index.ts` is the re-export entry. Monolith deleted; consumers' `import from '../api/report_data'` resolves to `report_data/index.ts` via Node module resolution. Largest sub-file: `timeline.ts` at 520 LOC (under the 600 cap). 53/53 report-data tests pass. Helper-extraction analysis (Task 11 — dedupe `resolveEventPlace`, `getPrimaryName`, etc. across the 7 files if ≥2 use identical shape) pending in a follow-up round. Roadmap §3.4.
