...
describe('holger profile — couple subtype', async () => {
  it('maps ENGA TYPE Sambo → cohabitation', async () => {
    const db = await createTestDb();
    await importGedcom(db, parseGedcom(HOLGER_SAMBO_GED), { profile: 'holger' });
  ...
  it('does not create an engagement event for Holger ENGA couples', async () => {
    const db = await createTestDb();
    await importGedcom(db, parseGedcom(HOLGER_SAMBO_GED), { profile: 'holger' });
  ...
  it('still uses marriage subtype for MARR families', async () => {
    const db = await createTestDb();
    await importGedcom(db, parseGedcom(HOLGER_MARR_GED), { profile: 'holger' });
    const row = db.get('SELECT subtype FROM relationships WHERE type=?', ['couple']) as { subtype: string } | undefined;
    expect(row?.subtype).toBe('marriage');
  });
});

describe('holger profile — MARR+ENGA', async () => {
  it('imports engagement event when FAM has both MARR and ENGA', async () => {
    const db = await createTestDb();
    await importGedcom(db, parseGedcom(HOLGER_MARR_AND_ENGA_GED), { profile: 'holger' });
    const row = db.get('SELECT COUNT(*) as n FROM events WHERE event_type=?', ['engagement']) as { n: number };
    expect(row.n).toBe(1);
  });
});
...
describe('holger profile — ADOP parent-child subtype', async () => {
  it('maps ADOP TYPE Fosterbarn → foster on parent_child relationships', async () => {
    const db = await createTestDb();
    await importGedcom(db, parseGedcom(HOLGER_FOSTER_GED), { profile: 'holger' });
...
describe('holger profile — media path remapping', async () => {
  it('remaps Windows OBJE FILE path to local mediaDir', async () => {
    const db = await createTestDb();
    await importGedcom(db, parseGedcom(HOLGER_MEDIA_GED), { profile: 'holger', mediaDir: '/local/Media' });
  ...
  it('keeps path as-is when no mediaDir provided', async () => {
    const db = await createTestDb();
    await importGedcom(db, parseGedcom(HOLGER_MEDIA_GED), { profile: 'holger' });
    const row = db.get('SELECT file_ref FROM media LIMIT 1') as { file_ref: string } | undefined;
...
describe('holger profile — REMA/MISC → notes, _HDP/_H8P suppression', async () => {
  it('imports REMA as person notes', async () => {
    const db = await createTestDb();
    await importGedcom(db, parseGedcom(HOLGER_REMA_GED), { profile: 'holger' });
  ...
  it('appends REMA to existing notes with double newline separator', async () => {
    const db = await createTestDb();
    await importGedcom(db, parseGedcom(HOLGER_REMA_EXISTING_NOTE_GED), { profile: 'holger' });
    const row = db.get('SELECT notes FROM persons LIMIT 1') as { notes: string } | undefined;
  ...
  it('imports MISC as person notes', async () => {
    const db = await createTestDb();
    await importGedcom(db, parseGedcom(HOLGER_MISC_GED), { profile: 'holger' });
    const row = db.get('SELECT notes FROM persons LIMIT 1') as { notes: string } | undefined;
    expect(row?.notes).toContain('Totalt 15 barn');
  });

  it('lists _HDP and _H8P in skipped tags (not silently ignored)', async () => {
    const db = await createTestDb();
    const report = await importGedcom(db, parseGedcom(HOLGER_REMA_GED), { profile: 'holger' });
    const tags = report.skipped.map(s => s.tag);
    expect(tags).toContain('_HDP');
    expect(tags).toContain('_H8P');
  });

  it('adds unmappedData entry for _HDP/_H8P with descriptive category', async () => {
    const db = await createTestDb();
    const report = await importGedcom(db, parseGedcom(HOLGER_REMA_GED), { profile: 'holger' });
  ...
  it('adds a warnings entry summarising REMA/MISC as imported notes', async () => {
    const db = await createTestDb();
    const report = await importGedcom(db, parseGedcom(HOLGER_REMA_GED), { profile: 'holger' });
...
describe('holger profile — defaultPersonId from first INDI', async () => {
  it('sets defaultPersonId to the DB id of the first INDI', async () => {
    const db = await createTestDb();
    const report = await importGedcom(db, parseGedcom(HOLGER_SUBM_GED), { profile: 'holger' });
...
describe('holger profile — ImportReport structure', async () => {
  it('returns ImportReport with correct persons and families counts', async () => {
    const db = await createTestDb();
    const report = await importGedcom(db, parseGedcom(HOLGER_MARR_AND_ENGA_GED), { profile: 'holger' });
  ...
  it('media is imported with correct file_ref when mediaDir is provided', async () => {
    const db = await createTestDb();
    await importGedcom(db, parseGedcom(HOLGER_MEDIA_GED), { profile: 'holger', mediaDir: '/local/Media' });
  ...
  it('media is imported even without mediaDir (path kept as-is)', async () => {
    const db = await createTestDb();
    const report = await importGedcom(db, parseGedcom(HOLGER_MEDIA_GED), { profile: 'holger' });
