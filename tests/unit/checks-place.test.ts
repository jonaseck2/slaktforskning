...
describe('ORPHANED_PLACE', async () => {
  it('fires for a place with no references', async () => {
    const pl = await createPlace(db, { name: 'Ingenstans' });
  ...
  it('does not fire when place is used by an event', async () => {
    const pl = await createPlace(db, { name: 'Använd plats' });
    const p = await createPerson(db, {}, { allowNameless: true });
    const e = await createEvent(db, { event_type: 'birth', place_id: pl.id });
  ...
  it('does not fire when place is a parent of another place', async () => {
    const parent = await createPlace(db, { name: 'Sverige' });
    await createPlace(db, { name: 'Stockholm', parent_place_id: parent.id });
    const results = await runAllChecks(db);
    expect(results.filter(r => r.code === 'ORPHANED_PLACE' && r.placeIds?.includes(parent.id))).toHaveLength(0);
  });
});

describe('CIRCULAR_PLACE_HIERARCHY', async () => {
  it('fires when three places form a cycle via parent_place_id', async () => {
    const a = await createPlace(db, { name: 'A' });
    const b = await createPlace(db, { name: 'B', parent_place_id: a.id });
    const c = await createPlace(db, { name: 'C', parent_place_id: b.id });
    ...
    await queryRun(db, 'UPDATE places SET parent_place_id = ? WHERE id = ?', [c.id, a.id]);
    const results = await runAllChecks(db);
  ...
  it('does not fire for a straight chain', async () => {
    const a = await createPlace(db, { name: 'Country' });
    const b = await createPlace(db, { name: 'Region', parent_place_id: a.id });
    await createPlace(db, { name: 'Parish', parent_place_id: b.id });
    const results = await runAllChecks(db);
    expect(results.filter(r => r.code === 'CIRCULAR_PLACE_HIERARCHY')).toHaveLength(0);
  });
});

describe('PLACE_COORDINATES_INVALID', async () => {
  it('fires when latitude is out of range', async () => {
    const pl = await createPlace(db, { name: 'Mars', latitude: 200, longitude: 10 });
    const results = await runAllChecks(db);
  ...
  it('fires for null-island (0, 0)', async () => {
    const pl = await createPlace(db, { name: 'NullIsland', latitude: 0, longitude: 0 });
    const results = await runAllChecks(db);
  ...
  it('does not fire for valid coordinates', async () => {
    const pl = await createPlace(db, { name: 'Stockholm', latitude: 59.3, longitude: 18.1 });
    const results = await runAllChecks(db);
  ...
  it('does not fire for missing coordinates', async () => {
    const pl = await createPlace(db, { name: 'NoCoords' });
    const results = await runAllChecks(db);
    expect(results.filter(r => r.code === 'PLACE_COORDINATES_INVALID' && r.placeIds?.includes(pl.id))).toHaveLength(0);
  });
});

describe('PLACE_NAME_LOOKS_LIKE_DATE', async () => {
  it('fires for a bare year', async () => {
    const pl = await createPlace(db, { name: '1736' });
    const hits = (await checkPlaceNameLooksLikeDate(db)).filter(r => r.placeIds?.includes(pl.id));
  ...
  it('fires for YYYY-MM-DD', async () => {
    const pl = await createPlace(db, { name: '1736-11-11' });
    const hits = (await checkPlaceNameLooksLikeDate(db)).filter(r => r.placeIds?.includes(pl.id));
    expect(hits).toHaveLength(1);
  });

  it('fires for YYYY-MM', async () => {
    const pl = await createPlace(db, { name: '1736-11' });
    const hits = (await checkPlaceNameLooksLikeDate(db)).filter(r => r.placeIds?.includes(pl.id));
    expect(hits).toHaveLength(1);
  });
  ...
  it('does not fire for a place name starting with a year', async () => {
    const pl = await createPlace(db, { name: '1736 Frederiksberg' });
    const hits = (await checkPlaceNameLooksLikeDate(db)).filter(r => r.placeIds?.includes(pl.id));
    expect(hits).toHaveLength(0);
  });

  it('does not fire for a normal place name', async () => {
    const pl = await createPlace(db, { name: 'Stockholm' });
    const hits = (await checkPlaceNameLooksLikeDate(db)).filter(r => r.placeIds?.includes(pl.id));
    expect(hits).toHaveLength(0);
  });

  it('does not fire for a 3-digit number (not a year)', async () => {
    const pl = await createPlace(db, { name: '123' });
    const hits = (await checkPlaceNameLooksLikeDate(db)).filter(r => r.placeIds?.includes(pl.id));
    expect(hits).toHaveLength(0);
  });
});

describe('PLACE_NAME_BROKEN_LANSBOKSTAV', async () => {
  it('fires for "Borås (PI" (broken trailing I)', async () => {
    const pl = await createPlace(db, { name: 'Borås (PI' });
    const hits = (await checkPlaceNameBrokenLansbokstav(db)).filter(r => r.placeIds?.includes(pl.id));
    expect(hits).toHaveLength(1);
  ...
  it('fires for "Hed (UI" (broken trailing I)', async () => {
    const pl = await createPlace(db, { name: 'Hed (UI' });
    const hits = (await checkPlaceNameBrokenLansbokstav(db)).filter(r => r.placeIds?.includes(pl.id));
    expect(hits).toHaveLength(1);
  ...
  it('fires for "Byske (ACI" (broken two-letter ACI)', async () => {
    const pl = await createPlace(db, { name: 'Byske (ACI' });
    const hits = (await checkPlaceNameBrokenLansbokstav(db)).filter(r => r.placeIds?.includes(pl.id));
    expect(hits).toHaveLength(1);
  ...
  it('fires for "Borås (P|" (broken pipe instead of paren)', async () => {
    const pl = await createPlace(db, { name: 'Borås (P|' });
    const hits = (await checkPlaceNameBrokenLansbokstav(db)).filter(r => r.placeIds?.includes(pl.id));
    expect(hits).toHaveLength(1);
    expect(hits[0].messageParams?.suggestion).toBe('Borås (P)');
  });
  ...
  it('does not fire for "Stockholm (A)" (clean parens)', async () => {
    const pl = await createPlace(db, { name: 'Stockholm (A)' });
    const hits = (await checkPlaceNameBrokenLansbokstav(db)).filter(r => r.placeIds?.includes(pl.id));
    expect(hits).toHaveLength(0);
  });

  it('does not fire for "Gotland (I)" (single I is a valid länsbokstav)', async () => {
    const pl = await createPlace(db, { name: 'Gotland (I)' });
    const hits = (await checkPlaceNameBrokenLansbokstav(db)).filter(r => r.placeIds?.includes(pl.id));
    expect(hits).toHaveLength(0);
  });
  ...
  it('does not fire for "(Approximate" (not a real länsbokstav)', async () => {
    ...
    const pl = await createPlace(db, { name: 'Foo (XYI' });
    const hits = (await checkPlaceNameBrokenLansbokstav(db)).filter(r => r.placeIds?.includes(pl.id));
    expect(hits).toHaveLength(0);
  });
...
describe('PLACE_DATES_INVERTED', async () => {
  it('fires when date_from is after date_to', async () => {
    const pl = await createPlace(db, { name: 'Bakvänd', date_from: '1900-01-01', date_to: '1850-01-01' });
    const results = await runAllChecks(db);
  ...
  it('does not fire when only one date is set', async () => {
    const pl = await createPlace(db, { name: 'Bara från', date_from: '1900-01-01' });
    const results = await runAllChecks(db);
  ...
  it('does not fire when dates are in order', async () => {
    const pl = await createPlace(db, { name: 'OK', date_from: '1850-01-01', date_to: '1900-01-01' });
    const results = await runAllChecks(db);
    expect(results.filter(r => r.code === 'PLACE_DATES_INVERTED' && r.placeIds?.includes(pl.id))).toHaveLength(0);
  });
});
