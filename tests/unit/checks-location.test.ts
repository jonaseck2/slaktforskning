...
describe('checkGazetteerMatchQuality', async () => {
  it('returns no results for places with manual coordinates', async () => {
    const place = await createPlace(db, { name: 'Okänd plats', latitude: 59.0, longitude: 18.0 });
  ...
  it('returns PLACE_MATCH_NONE for an unresolvable place linked to an event', async () => {
    const place = await createPlace(db, { name: 'Xyzzy, Fjärran Ort' });
    const person = await createPerson(db, {}, { allowNameless: true });
    const event = await createEvent(db, { event_type: 'birth', place_id: place.id });
    await addEventParticipant(db, { event_id: event.id, person_id: person.id });
    const results = await checkGazetteerMatchQuality(db, [testGazetteer]);
  ...
  it('returns PLACE_MATCH_WRONG_LEVEL when a single-word name matches a deep leaf', async () => {
    ...
    const place = await createPlace(db, { name: 'Amerika' });
    const person = await createPerson(db, {}, { allowNameless: true });
    const event = await createEvent(db, { event_type: 'emigration', place_id: place.id });
    await addEventParticipant(db, { event_id: event.id, person_id: person.id });
    const results = await checkGazetteerMatchQuality(db, [testGazetteer]);
  ...
  it('returns no issue for an exact match', async () => {
    const place = await createPlace(db, { name: 'Smedjebacken, Dalarnas län, Sverige' });
    const person = await createPerson(db, {}, { allowNameless: true });
    const event = await createEvent(db, { event_type: 'birth', place_id: place.id });
    await addEventParticipant(db, { event_id: event.id, person_id: person.id });
    const results = await checkGazetteerMatchQuality(db, [testGazetteer]);
  ...
  it('returns PLACE_MATCH_PARTIAL when there are unmatched components', async () => {
    ...
    const place = await createPlace(db, { name: 'Smedjebacken, Okänd Del, Sverige' });
    const person = await createPerson(db, {}, { allowNameless: true });
    const event = await createEvent(db, { event_type: 'birth', place_id: place.id });
    await addEventParticipant(db, { event_id: event.id, person_id: person.id });
    const results = await checkGazetteerMatchQuality(db, [testGazetteer]);
  ...
  it('does not populate personIds — place-match issues are place-scoped', async () => {
    const place = await createPlace(db, { name: 'Xyzzy, Nowhere' });
    const person1 = await createPerson(db, {}, { allowNameless: true });
    const person2 = await createPerson(db, {}, { allowNameless: true });
    const event = await createEvent(db, { event_type: 'birth', place_id: place.id });
    await addEventParticipant(db, { event_id: event.id, person_id: person1.id });
    await addEventParticipant(db, { event_id: event.id, person_id: person2.id });
    const results = await checkGazetteerMatchQuality(db, [testGazetteer]);
    const hit = results.filter(r => r.placeIds?.includes(place.id));
    expect(hit).toHaveLength(1);
  ...
  it('skips places not linked to any event', async () => {
    await createPlace(db, { name: 'Xyzzy, Nowhere' }); // no event
    const results = await checkGazetteerMatchQuality(db, [testGazetteer]);
    expect(results).toHaveLength(0);
  });
  ...
  it('checks places referenced by an event even if the event has no participants', async () => {
    ...
    await createEvent(db, { event_type: 'birth', place_id: place.id });
    const results = await checkGazetteerMatchQuality(db, [testGazetteer]);
    const hit = results.filter(r => r.placeIds?.includes(place.id));
    expect(hit).toHaveLength(1);
  });
  ...
  it('single-word place matching a leaf at depth > 2 triggers PLACE_MATCH_WRONG_LEVEL', async () => {
    ...
    const place = await createPlace(db, { name: 'Smedjebacken' });
    const person = await createPerson(db, {}, { allowNameless: true });
    const event = await createEvent(db, { event_type: 'birth', place_id: place.id });
    await addEventParticipant(db, { event_id: event.id, person_id: person.id });
    const results = await checkGazetteerMatchQuality(db, [testGazetteer]);
...
describe('checkPlaceMissingComma', async () => {
  it('returns empty array when no gazetteers are provided', async () => {
    const pl = await createPlace(db, { name: 'Richmond Kalifornien USA' });
    await createEvent(db, { event_type: 'birth', place_id: pl.id });
  ...
  it('does not fire on legit multi-word leaf names (no shallow anchor)', async () => {
    ...
    const pl = await createPlace(db, { name: 'Saint Mary' });
    await createEvent(db, { event_type: 'birth', place_id: pl.id });
  ...
  it('skips places not referenced by any event', async () => {
    await createPlace(db, { name: 'Richmond Kalifornien USA' });
    const results = await checkPlaceMissingComma(db, [usGazetteer, svGazetteer]);
    expect(results).toHaveLength(0);
  });

  it('does not flag a place with manual coordinates', async () => {
    const pl = await createPlace(db, { name: 'Richmond Kalifornien USA', latitude: 37.93, longitude: -122.34 });
    await createEvent(db, { event_type: 'birth', place_id: pl.id });
    const results = await checkPlaceMissingComma(db, [usGazetteer, svGazetteer]);
...
describe('checkPlaceNameNoRegion', async () => {
  it('returns empty array when no gazetteers are provided', async () => {
    const pl = await createPlace(db, { name: 'Stockhom' });
    await createEvent(db, { event_type: 'birth', place_id: pl.id });
  ...
  it('fires for a street address with no parent place', async () => {
    const pl = await createPlace(db, { name: 'Fredsgatan 16' });
    await createEvent(db, { event_type: 'birth', place_id: pl.id });
    const results = await checkPlaceNameNoRegion(db, [svGazetteer]);
    const hit = results.filter(r => r.code === 'PLACE_NAME_NO_REGION' && r.placeIds?.includes(pl.id));
    expect(hit).toHaveLength(1);
  ...
  it('does not fire when the place has a parent_place_id', async () => {
    ...
    const pl = await createPlace(db, { name: 'Hus', parent_place_id: parent.id });
    await createEvent(db, { event_type: 'birth', place_id: pl.id });
    const results = await checkPlaceNameNoRegion(db, [svGazetteer]);
    const hit = results.filter(r => r.code === 'PLACE_NAME_NO_REGION' && r.placeIds?.includes(pl.id));
    expect(hit).toHaveLength(0);
  ...
  it('skips places not referenced by any event', async () => {
    await createPlace(db, { name: 'Stockhom' });
    const results = await checkPlaceNameNoRegion(db, [svGazetteer]);
    expect(results).toHaveLength(0);
  });

  it('does not fire for a place with manual coordinates', async () => {
    const pl = await createPlace(db, { name: 'Stockhom', latitude: 59.0, longitude: 18.0 });
    await createEvent(db, { event_type: 'birth', place_id: pl.id });
    const results = await checkPlaceNameNoRegion(db, [svGazetteer]);
    expect(results.filter(r => r.placeIds?.includes(pl.id))).toHaveLength(0);
  });
