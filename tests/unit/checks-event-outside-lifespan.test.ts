...
function personWithBirth(date: string) {
  const p = createPerson(db, {}, { allowNameless: true });
  const birth = createEvent(db, { event_type: 'birth', date_type: 'exact', date_value: date });
  addEventParticipant(db, { event_id: birth.id, person_id: p.id, role: 'primary' });
  return { person: p, birth };
}

function addDeath(personId: string, date: string) {
  const death = createEvent(db, { event_type: 'death', date_type: 'exact', date_value: date });
  addEventParticipant(db, { event_id: death.id, person_id: personId, role: 'primary' });
  return death;
}
...
function addEvent(
  ...
  const ev = createEvent(db, { event_type: type, date_type, date_value: date });
  addEventParticipant(db, { event_id: ev.id, person_id: personId, role });
  return ev;
}

describe('EVENT_BEFORE_BIRTH', () => {
  it('fires when an event date is before the birth date (full ISO precision)', async () => {
    const { person } = personWithBirth('1944-06-15');
    const ev = addEvent(person.id, 'name_change', '1943-01-01');
  ...
  it('fires when event year is before birth year (year-only precision)', async () => {
    const { person } = personWithBirth('1944');
    const ev = addEvent(person.id, 'description', '1943');
    const results = await runAllChecks(db);
  ...
  it('does not fire when event year matches birth year (precision insufficient)', async () => {
    ...
    const { person } = personWithBirth('1944-06-15');
    addEvent(person.id, 'description', '1944');
    const results = await runAllChecks(db);
  ...
  it('uses the earliest birth event when multiple are recorded', async () => {
    ...
    const b1 = createEvent(db, { event_type: 'birth', date_type: 'exact', date_value: '1944-06-15' });
    addEventParticipant(db, { event_id: b1.id, person_id: p.id, role: 'primary' });
    const b2 = createEvent(db, { event_type: 'birth', date_type: 'exact', date_value: '1950-01-01' });
    addEventParticipant(db, { event_id: b2.id, person_id: p.id, role: 'primary' });
    // Event in 1945 is before 1950 but after 1944 — should NOT fire (we use earliest birth).
    addEvent(p.id, 'description', '1945');
    const results = await runAllChecks(db);
    const hit = results.filter((r) => r.code === 'EVENT_BEFORE_BIRTH' && r.personIds.includes(p.id));
    expect(hit).toHaveLength(0);
  });
  ...
  it('flags witness/non-primary participants too', async () => {
    const { person: focal } = personWithBirth('1900');
    const { person: witness } = personWithBirth('1950'); // born after the event date
    // An event in 1920 — focal is alive (born 1900), witness was not yet born.
    const ev = createEvent(db, { event_type: 'mention', date_type: 'exact', date_value: '1920' });
    addEventParticipant(db, { event_id: ev.id, person_id: focal.id, role: 'primary' });
...
describe('EVENT_OUTSIDE_LIFESPAN_AFTER_DEATH', () => {
  it('fires when an event date is after the death date', async () => {
    const { person } = personWithBirth('1900');
    addDeath(person.id, '1980');
    const ev = addEvent(person.id, 'description', '1985');
    const results = await runAllChecks(db);
    const hit = results.filter(
  ...
  it('does not fire when no death event is recorded', async () => {
    const { person } = personWithBirth('1900');
    addEvent(person.id, 'description', '2050');
    const results = await runAllChecks(db);
    const hit = results.filter(
  ...
  it('uses the latest death event when multiple are recorded', async () => {
    const { person } = personWithBirth('1900');
    addDeath(person.id, '1970');
    addDeath(person.id, '1980'); // latest
    addEvent(person.id, 'description', '1975'); // before 1980 → no fire
    const results = await runAllChecks(db);
    const hit = results.filter(
      (r) => r.code === 'EVENT_OUTSIDE_LIFESPAN_AFTER_DEATH' && r.personIds.includes(person.id),
    );
...
describe('runChecksForEvent (save-time hook)', () => {
  it('returns the warning row for an event saved before birth', () => {
    const { person } = personWithBirth('1944-06-15');
    const ev = addEvent(person.id, 'name_change', '1943-01-01');
    const results = runChecksForEvent(db, ev.id);
    expect(results).toHaveLength(1);
    expect(results[0].code).toBe('EVENT_BEFORE_BIRTH');
    expect(results[0].severity).toBe('warning');
  });

  it('returns the warning row for an event saved after death', () => {
    const { person } = personWithBirth('1900');
    addDeath(person.id, '1980');
    const ev = addEvent(person.id, 'description', '1985');
    const results = runChecksForEvent(db, ev.id);
  ...
  it('returns no rows for an in-lifespan event', () => {
    const { person } = personWithBirth('1900');
    addDeath(person.id, '1980');
    const ev = addEvent(person.id, 'residence', '1950');
    const results = runChecksForEvent(db, ev.id);
    expect(results).toHaveLength(0);
  });

  it('returns no rows for an event with no date_value', () => {
    const { person } = personWithBirth('1900');
    const ev = addEvent(person.id, 'mention', null);
    const results = runChecksForEvent(db, ev.id);
    expect(results).toHaveLength(0);
  });

  it('does not flag the birth event against itself', () => {
    const { person, birth } = personWithBirth('1944');
    const results = runChecksForEvent(db, birth.id);
    expect(results).toHaveLength(0);
  });

  it('flags an after-death event for ALL participants (witnesses, etc.)', () => {
    const { person: a } = personWithBirth('1900');
    addDeath(a.id, '1970'); // a died 1970
    const { person: b } = personWithBirth('1920');
    // Event in 1980 — a's death is 1970, b is alive.
    const ev = createEvent(db, { event_type: 'mention', date_type: 'exact', date_value: '1980' });
    addEventParticipant(db, { event_id: ev.id, person_id: a.id, role: 'primary' });
    addEventParticipant(db, { event_id: ev.id, person_id: b.id, role: 'witness' });
    const results = runChecksForEvent(db, ev.id);
