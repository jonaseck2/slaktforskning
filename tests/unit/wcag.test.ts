...
describe('parseHex', async () => {
  it('parses 6-digit hex with #', async () => {
    expect(parseHex('#ff8040')).toEqual([255, 128, 64]);
  });

  it('parses 6-digit hex without #', async () => {
    expect(parseHex('ff8040')).toEqual([255, 128, 64]);
  });

  it('parses 3-digit shorthand', async () => {
...
describe('relativeLuminance', async () => {
  ...
  it('is monotonic from black to white along the gray axis', async () => {
...
describe('contrastRatio', async () => {
  it('black on white = 21:1', async () => {
    expect(contrastRatio('#000000', '#ffffff')).toBeCloseTo(21, 1);
  });

  it('is symmetric (fg/bg order does not matter)', async () => {
    expect(contrastRatio('#000000', '#ffffff')).toBe(contrastRatio('#ffffff', '#000000'));
  });

  it('identical colors = 1:1', async () => {
    expect(contrastRatio('#777777', '#777777')).toBe(1);
    expect(contrastRatio('#abc123', '#abc123')).toBe(1);
  });

  // WebAIM reference: #777777 on #FFFFFF = 4.48:1
  it('matches known WebAIM reference (#777777 on #ffffff ≈ 4.48:1)', async () => {
    expect(contrastRatio('#777777', '#ffffff')).toBeCloseTo(4.48, 1);
  });

  // W3C AAA boundary example: #595959 on #ffffff ≈ 7.0:1
  it('matches W3C AAA boundary (#595959 on #ffffff ≈ 7.0:1)', async () => {
