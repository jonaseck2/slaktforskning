...
describe('MEDIA_FILE_MISSING (relocated)', async () => {
  it('still fires when is_missing flag is 1', async () => {
    const m = await createMedia(db, { title: 'Missing photo', file_ref: '/absent.jpg' });
    await queryRun(db, 'UPDATE media SET is_missing = 1 WHERE id = ?', [m.id]);
...
describe('ORPHANED_MEDIA', async () => {
  it('fires for media with no links', async () => {
    const m = await createMedia(db, { title: 'Lonely photo' });
    const results = await runAllChecks(db);
  ...
  it('does not fire for media linked to a person', async () => {
    const m = await createMedia(db, { title: 'Linked photo' });
    const p = await createPerson(db, {}, { allowNameless: true });
    await addMediaLink(db, { media_id: m.id, entity_type: 'person', entity_id: p.id });
    const results = await runAllChecks(db);
...
describe('MEDIA_REGION_OUT_OF_BOUNDS', async () => {
  it('fires when a region extends past the right edge', async () => {
    const m = await createMedia(db, { title: 'Photo' });
    const p = await createPerson(db, {}, { allowNameless: true });
    await addMediaLink(db, { media_id: m.id, entity_type: 'person', entity_id: p.id });
    const region = await createMediaRegion(db, { media_id: m.id, person_id: p.id, x: 0.8, y: 0.1, width: 0.5, height: 0.2 });
    const results = await runAllChecks(db);
...
describe('PHOTO_AFTER_SUBJECT_DEATH', async () => {
  it('fires when a tagged person died before the linked event date', async () => {
    ...
    // Death 1900
    const death = await createEvent(db, { event_type: 'death', date_type: 'exact', date_value: '1900-06-01' });
    await addEventParticipant(db, { event_id: death.id, person_id: p.id, role: 'primary' });

    // Media linked to event in 1950
    const m = await createMedia(db, { title: 'Family reunion' });
    const event = await createEvent(db, { event_type: 'reunion', date_type: 'exact', date_value: '1950-07-01' });
    await addMediaLink(db, { media_id: m.id, entity_type: 'event', entity_id: event.id });
    await createMediaRegion(db, { media_id: m.id, person_id: p.id, x: 0.1, y: 0.1, width: 0.2, height: 0.2 });
  ...
  it('does not fire when event date is before death', async () => {
    const p = await createPerson(db, {}, { allowNameless: true });
    const death = await createEvent(db, { event_type: 'death', date_type: 'exact', date_value: '1950-01-01' });
    await addEventParticipant(db, { event_id: death.id, person_id: p.id, role: 'primary' });

    const m = await createMedia(db, { title: 'Early photo' });
    const event = await createEvent(db, { event_type: 'portrait', date_type: 'exact', date_value: '1940-01-01' });
    await addMediaLink(db, { media_id: m.id, entity_type: 'event', entity_id: event.id });
    await createMediaRegion(db, { media_id: m.id, person_id: p.id, x: 0.1, y: 0.1, width: 0.2, height: 0.2 });
  ...
  it('does not fire for floating media without event links', async () => {
    const p = await createPerson(db, {}, { allowNameless: true });
    const death = await createEvent(db, { event_type: 'death', date_type: 'exact', date_value: '1900-01-01' });
    await addEventParticipant(db, { event_id: death.id, person_id: p.id, role: 'primary' });

    const m = await createMedia(db, { title: 'Floating photo' });
    await addMediaLink(db, { media_id: m.id, entity_type: 'person', entity_id: p.id });
    await createMediaRegion(db, { media_id: m.id, person_id: p.id, x: 0.1, y: 0.1, width: 0.2, height: 0.2 });
...
describe('PHOTO_BEFORE_SUBJECT_BIRTH', async () => {
  it('fires when a tagged person was born after the linked event date', async () => {
    const p = await createPerson(db, {}, { allowNameless: true });
    const birth = await createEvent(db, { event_type: 'birth', date_type: 'exact', date_value: '1950-01-01' });
    await addEventParticipant(db, { event_id: birth.id, person_id: p.id, role: 'primary' });

    const m = await createMedia(db, { title: 'Old photo' });
    const event = await createEvent(db, { event_type: 'portrait', date_type: 'exact', date_value: '1900-01-01' });
    await addMediaLink(db, { media_id: m.id, entity_type: 'event', entity_id: event.id });
    await createMediaRegion(db, { media_id: m.id, person_id: p.id, x: 0.1, y: 0.1, width: 0.2, height: 0.2 });
  ...
  it('does not fire when event date is after birth', async () => {
    ...
    const m = await createMedia(db, { title: 'Photo' });
    const event = await createEvent(db, { event_type: 'portrait', date_type: 'exact', date_value: '1960-01-01' });
    await addMediaLink(db, { media_id: m.id, entity_type: 'event', entity_id: event.id });
    await createMediaRegion(db, { media_id: m.id, person_id: p.id, x: 0.1, y: 0.1, width: 0.2, height: 0.2 });
