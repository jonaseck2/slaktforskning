...
describe('findDuplicateSources', async () => {
  it('returns nothing on an empty DB', async () => {
    expect(await findDuplicateSources(db)).toEqual([]);
    expect(await countDuplicateSources(db)).toBe(0);
  });

  it('finds sources with identical normalised titles + same author at the highest score', async () => {
    // Score-100 canary: trailing-space-only difference normalises to equal.
    const a = await createSource(db, { title: 'Adolf Fredrik C:I:6, 1798-1812', author: 'Pastor Eriksson' });
    const b = await createSource(db, { title: 'Adolf Fredrik C:I:6, 1798-1812 ', author: 'Pastor Eriksson' });
    const dupes = await findDuplicateSources(db);
  ...
  it('finds the Adolf-Fredrik near-duplicate (em-dash vs hyphen) — user-goal canary', async () => {
  ...
  it('finds titles within Levenshtein distance 2', async () => {
    await createSource(db, { title: 'A Genealogical History', author: 'Doe, J.' });
    await createSource(db, { title: 'A Genealogical Histor', author: 'Doe, J.' });
    const dupes = await findDuplicateSources(db);
    expect(dupes).toHaveLength(1);
    expect(dupes[0].reasons).toContain('levenshtein_1');
  });
  ...
  it('does not flag pairs beyond distance 2', async () => {
    await createSource(db, { title: 'A Genealogical History', author: 'Doe, J.' });
    await createSource(db, { title: 'Completely Different Book Title', author: 'Doe, J.' });
    expect(await findDuplicateSources(db)).toEqual([]);
  });

  it('does not flag sources with different authors even if titles match exactly', async () => {
    await createSource(db, { title: 'Same Title', author: 'Author A' });
    await createSource(db, { title: 'Same Title', author: 'Author B' });
    expect(await findDuplicateSources(db)).toEqual([]);
  });

  it('treats both-author-empty as a matching bucket', async () => {
    await createSource(db, { title: 'Untitled Manuscript' });
    await createSource(db, { title: 'Untitled Manuscript' });
    const dupes = await findDuplicateSources(db);
    expect(dupes).toHaveLength(1);
  ...
  it('does not treat empty-author and a real-author as the same bucket', async () => {
    await createSource(db, { title: 'Same Title' });
    await createSource(db, { title: 'Same Title', author: 'Doe, J.' });
    expect(await findDuplicateSources(db)).toEqual([]);
  });

  it('normalises author whitespace + case for grouping', async () => {
    await createSource(db, { title: 'Source A', author: 'doe,  J.' });
    await createSource(db, { title: 'Source A', author: 'DOE, J.' });
    expect(await findDuplicateSources(db)).toHaveLength(1);
  });

  it('skips rows with empty titles', async () => {
    await createSource(db, { title: '', author: 'X' });
    await createSource(db, { title: '', author: 'X' });
    expect(await findDuplicateSources(db)).toEqual([]);
  });

  it('honours ignored pairs', async () => {
    const a = await createSource(db, { title: 'Stockholm', author: 'X' });
    const b = await createSource(db, { title: 'Stockholm', author: 'X' });
    expect(await findDuplicateSources(db)).toHaveLength(1);
    await ignoreDuplicateSource(db, a.id, b.id);
    expect(await findDuplicateSources(db)).toHaveLength(0);
  });

  it('ignoreDuplicateSource is idempotent and rejects self-pairs', async () => {
    const a = await createSource(db, { title: 'X', author: 'A' });
    await expect(ignoreDuplicateSource(db, a.id, a.id)).rejects.toThrow();
    const b = await createSource(db, { title: 'X', author: 'A' });
    await ignoreDuplicateSource(db, a.id, b.id);
    await ignoreDuplicateSource(db, a.id, b.id);
  ...
  it('honours pagination via limit/offset', async () => {
    ...
    for (const author of authors) {
      await createSource(db, { title: 'Book', author });
      await createSource(db, { title: 'Book', author });
    }
    expect(await countDuplicateSources(db)).toBe(4);
    expect(await findDuplicateSources(db, 2, 0)).toHaveLength(2);
    expect(await findDuplicateSources(db, 2, 2)).toHaveLength(2);
    expect(await findDuplicateSources(db, 2, 4)).toHaveLength(0);
  });

  it('does not pair a source against persons of the same UUID by mistake', async () => {
    const a = await createSource(db, { title: 'X', author: 'A' });
    const b = await createSource(db, { title: 'X', author: 'A' });
  ...
  it('does not pair a source against a place-typed ignored row', async () => {
    const a = await createSource(db, { title: 'X', author: 'A' });
    const b = await createSource(db, { title: 'X', author: 'A' });
    const [low, high] = a.id < b.id ? [a.id, b.id] : [b.id, a.id];
    await runSql(db,
  ...
  it('returns highest-score pair first (sort by descending score)', async () => {
    await createSource(db, { title: 'Exact Match', author: 'A' });
    await createSource(db, { title: 'Exact Match', author: 'A' });
    await createSource(db, { title: 'Fuzzy Match', author: 'B' });
    await createSource(db, { title: 'Fuzy Match', author: 'B' });
    const dupes = await findDuplicateSources(db);
...
describe('mergeSources — repointing', async () => {
  it('repoints citations.source_id', async () => {
    const target = await createSource(db, { title: 'Stockholm', author: 'A' });
    const source = await createSource(db, { title: 'Stockholm', author: 'A' });
    const cit = await createCitation(db, { source_id: source.id });
    const result = await mergeSources(db, target.id, source.id);
    expect(result.moved.citations).toBe(1);
    expect((await getCitation(db, cit.id))?.source_id).toBe(target.id);
    expect(await getSource(db, source.id)).toBeNull();
  });

  it('repoints source_repositories rows and dedupes against target', async () => {
    const target = await createSource(db, { title: 'X', author: 'A' });
    const source = await createSource(db, { title: 'X', author: 'A' });
    const r1 = await createRepository(db, { name: 'r1' });
    const r2 = await createRepository(db, { name: 'r2' });
    await linkSourceRepository(db, source.id, r1.id);
    await linkSourceRepository(db, target.id, r2.id);
    await linkSourceRepository(db, source.id, r2.id);

    const result = await mergeSources(db, target.id, source.id);
    expect(result.moved.source_repositories).toBe(1);

    const onTarget = await getRepositoriesForSource(db, target.id);
    expect(onTarget.map(r => r.id).sort()).toEqual([r1.id, r2.id].sort());
    const orphans = await queryAll(db, 'SELECT * FROM source_repositories WHERE source_id = ?', [source.id]);
  ...
  it('repoints media_links and skips duplicates', async () => {
    ...
    await addMediaLink(db, { media_id: m1.id, entity_type: 'source', entity_id: source.id });
    await addMediaLink(db, { media_id: m2.id, entity_type: 'source', entity_id: target.id });
    await addMediaLink(db, { media_id: m2.id, entity_type: 'source', entity_id: source.id });

    const result = await mergeSources(db, target.id, source.id);
    expect(result.moved.media_links).toBe(1);

    const onTarget = await getMediaForEntity(db, 'source', target.id);
  ...
  it('throws on self-merge', async () => {
    const a = await createSource(db, { title: 'X', author: 'A' });
    await expect(mergeSources(db, a.id, a.id)).rejects.toThrow('Cannot merge a source with itself');
  });

  it('throws when target missing', async () => {
    const a = await createSource(db, { title: 'X', author: 'A' });
    await expect(mergeSources(db, 'no-such-id', a.id)).rejects.toThrow('Target source not found');
  });

  it('throws when source missing', async () => {
    const a = await createSource(db, { title: 'X', author: 'A' });
    await expect(mergeSources(db, a.id, 'no-such-id')).rejects.toThrow('Source source not found');
  });

  it('drops candidate pair count by one after merge', async () => {
    await createSource(db, { title: 'X', author: 'A' });
    const a = await createSource(db, { title: 'X', author: 'A' });
    const b = await createSource(db, { title: 'X', author: 'A' });
    expect(await countDuplicateSources(db)).toBeGreaterThanOrEqual(1);
    await mergeSources(db, a.id, b.id);
    expect(await countDuplicateSources(db)).toBe(1);
  });
});

describe('mergeSources — undo round-trip', async () => {
  it('restores every touched row exactly to its pre-merge state', async () => {
    const target = await createSource(db, { title: 'X', author: 'A' });
    const source = await createSource(db, { title: 'X', author: 'A' });
    const sourceSnapshot = await getSource(db, source.id)!;

    const cit = await createCitation(db, { source_id: source.id, page: 'p3' });
    const r1 = await createRepository(db, { name: 'r1' });
    const r2 = await createRepository(db, { name: 'r2' });
    await linkSourceRepository(db, source.id, r1.id);
    await linkSourceRepository(db, target.id, r2.id);
    ...
    const otherSource = await createSource(db, { title: 'Other', author: 'A' });
    await ignoreDuplicateSource(db, source.id, otherSource.id);

    const repoLinksBefore = await queryAll(db,
    ...
    await mergeSources(db, target.id, source.id);
    expect(await getSource(db, source.id)).toBeNull();
    expect((await getCitation(db, cit.id))?.source_id).toBe(target.id);
    ...
    const sourceAfter = await getSource(db, source.id);
    expect(sourceAfter).not.toBeNull();
    expect(sourceAfter).toEqual(sourceSnapshot);

    expect((await getCitation(db, cit.id))?.source_id).toBe(source.id);

    const repoLinksAfter = await queryAll(db,
      'SELECT source_id, repository_id FROM source_repositories ORDER BY source_id, repository_id'
    );
  ...
  it('redo replays the merge', async () => {
    ...
    expect(await getSource(db, source.id)).not.toBeNull();
    expect((await getCitation(db, cit.id))?.source_id).toBe(source.id);
...
describe('deleteSource — ignored_duplicates cleanup', async () => {
  it('removes source-typed ignored pairs that mention the deleted id', async () => {
    const a = await createSource(db, { title: 'X', author: 'A' });
    const b = await createSource(db, { title: 'X', author: 'A' });
    const c = await createSource(db, { title: 'Y', author: 'A' });
    await ignoreDuplicateSource(db, a.id, b.id);
    await ignoreDuplicateSource(db, a.id, c.id);
    expect(await queryAll(db, "SELECT * FROM ignored_duplicates WHERE entity_type = 'source'")).toHaveLength(2);
    await deleteSource(db, a.id);
    expect(await queryAll(db, "SELECT * FROM ignored_duplicates WHERE entity_type = 'source'")).toHaveLength(0);
  });

  it('does not touch person-typed or place-typed ignored rows', async () => {
    const a = await createSource(db, { title: 'X', author: 'A' });
    await runSql(db,
      "INSERT INTO ignored_duplicates (entity_type, person1_id, person2_id) VALUES ('person', ?, ?)",
