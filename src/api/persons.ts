...
export function deletePerson(db: Database.Database, id: string): boolean {
  const result = db.prepare(`DELETE FROM persons WHERE id = ?`).run(id);
  return result.changes > 0;
}

export function searchPersons(db: Database.Database, query: string): (Person & { given_name: string; surname: string })[] {
  const like = `%${query}%`;
  return db.prepare(`
    SELECT p.*, pn.given_name, pn.surname
    FROM persons p
...
export function getPersonNames(db: Database.Database, personId: string): PersonName[] {
