...
export function searchPersons(
  ...
  const birthYearSubquery = `(SELECT CAST(SUBSTR(e.date_value, 1, 4) AS INTEGER)
       FROM events e
       ...
       ORDER BY e.date_value LIMIT 1)`;

  const birthYearClauses: string[] = [];
  const birthYearParams: number[] = [];
  if (birthYearMin != null) { birthYearClauses.push(`${birthYearSubquery} >= ?`); birthYearParams.push(birthYearMin); }
  if (birthYearMax != null) { birthYearClauses.push(`${birthYearSubquery} <= ?`); birthYearParams.push(birthYearMax); }
  const whereExtra = birthYearClauses.length > 0 ? `AND ${birthYearClauses.join(' AND ')}` : '';
