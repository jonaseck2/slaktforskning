...
export function deleteFamily(db: Database.Database, id: string): boolean {
...
export function getChildrenOfFamily(db: Database.Database, familyId: string): PersonFamilyLink[] {
  return db.prepare(`SELECT * FROM person_family_links WHERE family_id = ?`).all(familyId) as PersonFamilyLink[];
}

export function getFamiliesOfPerson(db: Database.Database, personId: string): Family[] {
  return db.prepare(`
