<template>
  <div>
    <SectionEmpty
      ...
      purpose-key="onboarding.empty.relationshipSources.purpose"
      :action-label-key="props.readonly ? undefined : 'onboarding.empty.relationshipSources.cta'"
    ...
    <ConfirmModal
      ...
      :message="$t('relationshipSources.confirmDelete')"
      tone="danger"
      icon="⚠️"
...
const { data, reload } = useEntityData<CitationRow[]>(idRef, async (id) => {
  const raw = await window.api.citations.forRelationship(id) as CitationRow[];
...
const del = useDeleteConfirm<string>(async (id) => {
  try {
    ...
    console.error('[RelationshipSourcesSection] delete failed:', err);
