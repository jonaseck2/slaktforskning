<template>
  <div>
    <SectionEmpty
      ...
      purpose-key="onboarding.empty.placeSources.purpose"
      :action-label-key="props.readonly ? undefined : 'onboarding.empty.placeSources.cta'"
    ...
    <ConfirmModal
      ...
      :message="$t('placeSources.confirmDelete')"
      tone="danger"
      icon="⚠️"
...
const { data, reload } = useEntityData<CitationRow[]>(idRef, async (id) => {
  const raw = await window.api.citations.forPlace(id) as CitationRow[];
...
const del = useDeleteConfirm<string>(async (id) => {
  try {
    ...
    console.error('[PlaceSourcesSection] delete failed:', err);
