...
const api = {
  persons: {
    ...
    listPage: (
      ...
      sortBy2?: string | null,
      sortDir2?: 'asc' | 'desc',
    ) => ipcRenderer.invoke('persons:listPage', limit, offset, sortBy, sortDir, query, sortBy2 ?? null, sortDir2),
    ...
    refreshQualityIssueCounts: (counts: Record<string, number>) => ipcRenderer.invoke('persons:refreshQualityIssueCounts', counts),
    getQualityIssueCounts: (personIds: string[]) => ipcRenderer.invoke('persons:getQualityIssueCounts', personIds),
  },
  relationships: {
  ...
  sources: {
    ...
    listPage: (limit: number, offset: number, sortBy?: 'title' | 'author' | 'source_type', sortDir?: 'asc' | 'desc', query?: string) => ipcRenderer.invoke('sources:listPage', limit, offset, sortBy, sortDir, query),
  ...
  places: {
    ...
    listPage: (limit: number, offset: number, sortBy?: 'name' | 'place_type', sortDir?: 'asc' | 'desc', query?: string) => ipcRenderer.invoke('places:listPage', limit, offset, sortBy, sortDir, query),
  ...
  duplicates: {
    ...
    findPage: (limit?: number, offset?: number) => ipcRenderer.invoke('duplicates:findPage', limit, offset),
    ...
    findPlaces: (limit?: number, offset?: number) => ipcRenderer.invoke('duplicates:findPlaces', limit, offset),
    countPlaces: () => ipcRenderer.invoke('duplicates:countPlaces'),
    ignorePlace: mutating((placeAId: string, placeBId: string) => ipcRenderer.invoke('duplicates:ignorePlace', placeAId, placeBId)),
    mergePlaces: mutating((targetId: string, sourceId: string) => ipcRenderer.invoke('duplicates:mergePlaces', targetId, sourceId)),
    findSources: (limit?: number, offset?: number) => ipcRenderer.invoke('duplicates:findSources', limit, offset),
    countSources: () => ipcRenderer.invoke('duplicates:countSources'),
    ignoreSource: mutating((sourceAId: string, sourceBId: string) => ipcRenderer.invoke('duplicates:ignoreSource', sourceAId, sourceBId)),
    mergeSources: mutating((targetId: string, sourceId: string) => ipcRenderer.invoke('duplicates:mergeSources', targetId, sourceId)),
    findMedia: (limit?: number, offset?: number) => ipcRenderer.invoke('duplicates:findMedia', limit, offset),
    countMedia: () => ipcRenderer.invoke('duplicates:countMedia'),
    ignoreMedia: mutating((mediaAId: string, mediaBId: string) => ipcRenderer.invoke('duplicates:ignoreMedia', mediaAId, mediaBId)),
    mergeMedia: mutating((targetId: string, sourceId: string, keepFile: 'target' | 'source') => ipcRenderer.invoke('duplicates:mergeMedia', targetId, sourceId, keepFile)),
  },
  checks: {
  ...
  media: {
    ...
    thumbnailDataUrl: (fileRef: string, maxWidth?: number) => ipcRenderer.invoke('media:thumbnailDataUrl', fileRef, maxWidth),
