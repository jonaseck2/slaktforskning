...
- **Persist implementation plans** — Write plans to `docs/plans/` immediately (e.g. `docs/plans/YYYY-MM-DD-description.md`) and **commit the plan file in the same step**. Plans that are saved but not committed get orphaned on the source branch the moment a worktree is created (the worktree branches from HEAD and the uncommitted plan is left behind). Same rule for specs in `docs/plans/`. Context can be lost. When asked to "continue according to plan", look there for the most recent plan file.
...
- **Archive completed design specs** — Design specs live in `docs/plans/`. When a spec is fully implemented, move it to `docs/plans/archive/`. Update any `docs/PLAN.md` spec references to the archived path. Add a `[done]` roadmap entry with the archived spec pointer if one doesn't exist.
