...
- **Archive completed plans** — When a milestone is fully complete, move its plan file from `.claude/plans/` to `.claude/plans/archive/`. Update the `.claude/PLAN.md` pointer to the archived path. This prevents completed plans from being re-read in future sessions.
