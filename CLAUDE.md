...
`src/api/` is the single source of truth for all business logic. It is **runtime-neutral TypeScript** — runs in the renderer process via the Tauri DB shim. Both the renderer's `tauri-window-api.ts` (which binds each `window.api.<domain>.<method>` directly to a `src/api/*` function, alongside Rust-backed commands from the Specta-generated `src/renderer/bindings.ts`) and the MCP server (`src/mcp/server.ts`) call the same api/ functions. All api/ functions take a `Database` instance as their first argument (dependency injection, no singletons).
...
│                         # plus tauri-window-api.ts (the window.api wiring; inlines every
│                         # `domain:method` → api/* binding), bindings.ts (Specta-generated
│                         # auto-source-of-truth for Rust commands; regenerated by cargo build),
│                         # and db-shim.ts
...
0. **Produce evidence the Verification §1 criteria are met** — paste actual output (test counts, exit codes, MCP-call results, build-tail line) into the close-out commit message or a one-shot session note. Per [`.claude/rules/plans.md`](.claude/rules/plans.md) "Verification discipline at close-out": *"Tests should pass after this change"* is not evidence; the tail of `npm test` showing N passed is. *"`npm run build` should work — only docs changed"* is not evidence; the tail of `npm run build` exiting 0 is. Invoke `superpowers:verification-before-completion` explicitly. **If any user-observable command in the plan's Verification section hasn't been run with output captured, the plan is not ready to archive.** This step is non-negotiable; every Tauri-port-style "archived with broken `npm start`" failure originates here. The expected evidence template:
   - `npm test` → `N passed (Xs)` (paste the summary line).
   - `npm run build` → `built in Xs` (paste the tail line + exit code).
   - `npm run test:e2e` → `N passed (Xs)` across the 4 Tier 1 projects (`[boot]`, `[crud]`, `[website-export]`, `[duplicates]`) (paste the summary line).
   - `npm run test:e2e:full` → `N passed (Xs)` across the 7 projects (the 4 Tier 1 above plus `[panels]`, `[reactivity]`, `[imports]`) — **required for any plan whose user goal touches a panel, modal, list-view, importer, or `data-changed` consumer**. Non-UI plans (Rust-side only, schema-only, doc-only) are exempt.
...
3. Final version bump (largest change shipped: any feature → minor; fix-only refactor → patch) and a new CHANGELOG.md block summarising the plan. Block structure, bullet style, trim rules, archive flow: `oss-release` skill. Version-bump mechanics: `commit` skill.
...
6. Merge / push: plan-driven work can land via PR **or** via direct merge to `main` from the worktree — both paths are fine, your call. The rule is not "which path", it is **what verification ran before push**. Per [`.claude/rules/plans.md`](.claude/rules/plans.md) "CI catches PRs, the executor catches direct-to-main pushes": if you go through a PR, CI is the verification surface and you iterate on red. If you merge the worktree branch into `main` and push directly (the common case for solo plan-driven work here), the executor has already run `npm test`, `npm run build`, and the appropriate e2e tier locally and captured the output in the commit message. Both paths are fine; pushing to `origin/main` without local-green-before-push (or merging a red PR) is what's never OK. **And** — when merging the worktree to `main`, push `main` itself, not the feature branch — the Tauri full-port close-out merged 75 commits to local `main` then sat unpushed for the session, and a later session pushed the feature branch to `origin/main` instead of `main` itself; both are failure modes this rule guards against.
