...
- **Keep plans and roadmap in sync** — When adding a plan or spec to `docs/plans/`, add a matching milestone to `docs/PLAN.md` roadmap with a pointer. When completing a milestone, mark it done in both the plan file (checkboxes) and `docs/PLAN.md`. The roadmap is the index; the plan files are the detail.
- **Archive completed plans and specs** — When a milestone is fully complete, move its plan/spec file from `docs/plans/` to `docs/plans/archive/`. Update the `docs/PLAN.md` pointer to the archived path. This prevents completed files from being loaded into context unnecessarily. `docs/PLAN.md` Done entries are brief summaries only — full details live in the archived files.
