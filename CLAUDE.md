...
6. Merge / push: plan-driven work should land via PR (open the PR, let CI run, merge after green); direct-to-`main` archive is allowed only for "small fixes" (see below) and then **only after** running every check CI will run *locally first* and capturing the exit codes + test counts. Per [`.claude/rules/plans.md`](.claude/rules/plans.md) "CI catches PRs, the executor catches direct-to-main pushes": CI is the verification surface for PRs; for direct-to-main pushes it runs *after* the broken commit is in the protected branch, so the executor is the only check. The Tauri full-port close-out merged 75 commits to local `main` and sat unpushed for the session; the never-pushed state plus a then-broken `ci.yml` produced zero verification of the merged state. Never again.
