...
- [x] **Step 1:** Run the six-field form for each of these rules before writing the file:
...
- [x] **Step 2:** Write `.claude/rules/plans.md` covering A1–A4. Include a worked example: the panel-composables refactor *as it should have been written* (one paragraph each section, not full plan). Cap file at 150 lines.

- [x] **Step 3:** Verification artifact: re-read this very plan against the rules. Confirm it complies. If it doesn't, the rule is wrong, not the plan.

- [x] **Step 4:** Commit `docs(rules): plan authoring requires user goal + full pattern scope + verification`.
...
- [x] **Step 1:** Run the six-field form for each rule before writing:
...
- [x] **Step 2:** Write the SKILL.md (~80 lines) explaining when this skill activates: replaces the upstream `superpowers:subagent-driven-development` prompt templates for this project. SKILL.md cites the upstream skill but uses our local templates.

- [x] **Step 3:** Write the three prompt templates. Each one differs from upstream in one specific way per the rules above. Don't rewrite the whole template — fork minimally so future upstream improvements still apply.

- [x] **Step 4:** Verification artifact: dry-run a fake subagent dispatch using the new templates. Walk through what the implementer/spec-reviewer/code-quality-reviewer prompts produce on a hypothetical "migrate the X panels" task. Confirm pushback paths exist where they didn't before.

- [x] **Step 5:** Commit `docs(skill): subagent-handoff — user-goal-first prompts + dispatcher verification rule`.
...
- [x] **Step 1:** Run the six-field form for the single rule:
...
- [x] **Step 2:** Write `.claude/skills/dom-first-debugging/SKILL.md` (~50 lines). Include:
  - The rule.
...
- [x] **Step 3:** Verification artifact: cite the panel-composables RCA inline. The 320px collision was visible in 30 seconds of DOM inspection. The skill names this as the canonical failure case to learn from.

- [x] **Step 4:** Commit `docs(skill): dom-first-debugging — read truth before reasoning about CSS`.
...
- [x] **Step 1:** In `.claude/rules/renderer.md`, add a one-line pointer at the top: "Plan authoring → see `.claude/rules/plans.md`. Layout debugging → use `dom-first-debugging` skill."
- [x] **Step 2:** In `CLAUDE.md` "Workflow" section, add a one-liner pointing at `.claude/rules/plans.md` for plan format.
- [x] **Step 3:** Commit `docs: cross-link new plan-authoring rule + dom-debug skill`.
...
- [x] **Step 1:** Read the panel-consistency-finish plan against rules A1–A4. Note every gap (e.g. is "User goal" section explicit enough? Is the migration-target list complete? Is verification user-observable?).
- [x] **Step 2:** Patch the plan to comply. If the plan can't comply without restructuring, that's evidence the rules are correct.
- [x] **Step 3:** Commit `docs(plans): apply new plan-authoring rules to panel-consistency-finish`.

---

## Verification (user-observable outcomes, per Rule A3)

- [x] The next plan I write — for any feature, not just panels — opens with a "User goal" section that names the user-observable outcome before any technical detail. (Audit by re-reading next plan.)
- [x] The next subagent dispatch in this repo uses the local prompt templates and pushes back when scope deviates from user goal. (Audit by reading subagent reports.)
- [x] On the next layout / visual bug, the first tool call is a DOM read, not a CSS edit. (Audit by reading session transcript.)
- [x] The panel-consistency-finish plan, after Task 5, reads like a worked example of the new template — not a retrofit.
