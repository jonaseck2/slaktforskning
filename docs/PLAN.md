...
- **Hourglass layout internal refactor (Phase 2 of 3.2)** — no chart feature is blocked; the file works (1,052 LOC) and 130/130 layout tests pass. The premise was tested and held false: the `timeline` chart shipped (2026-06-07 re-audit) needing nothing from `hourglass.ts`, and hourglass is structurally the union of pedigree + descendant rather than a different algorithm — the only real shared duplication is cross-chart (`findPersonInTree`, placeholder-extraction tail), not hourglass-internal. **Reopen trigger:** a specific chart-feature request blocked by the file's shape. **Prune ~2026-11-14** if no such trigger appears. Design lives at [`plans/2026-05-14-hourglass-layout-refactor-design.md`](plans/2026-05-14-hourglass-layout-refactor-design.md).
