...
#### European Gazetteer Roadmap [done — 14/14 plans landed]
End-to-end European place-resolution coverage. All 14 plans landed; details in `docs/plans/archive/PLAN.md`. Bundled gazetteer count 29 → 72 (v0.229.0 → v0.241.0). europe-historical shipped as a sparse first cut (39 entities — Crown lands of Austria + Soviet Union republics) due to Wikidata SPARQL rate-limiting; the build script's per-class graceful-fallback supports re-runs to fill in the other 4 classes (Russian Empire governorates, Provinces of Prussia, HRE Imperial Estates, more historical countries) once Wikidata is responsive.
- Per-country follow-ups deferred (not part of the roadmap): Tier 1 parishes (UK civil + CofE, IE Catholic + townlands, NL historical gemeenten, BE Catholic, EE/LV Lutheran, LT/PL Catholic), FR ~35k communes, ru-eu / tr-eu (longitude-filtered), Tier 1 boundaries for IE/NL/BE/FR/EE/LV/LT/PL, German Confederation Bund states + generic "historical region of country" (Wikidata class structure unclear).
