...
export default defineConfig({
  ...
  plugins: [
    {
      // Gzip every bundled gazetteer JSON and emit at
      // <bundle-dir>/gazetteers/<id>.json.gz. The runtime loader in
      // src/api/place-gazetteers/bundled.ts loads them via
      // gunzipSync(readFileSync(...)) at module init. Source code holds no
      // static .json imports, so no resolveId rewrite is needed.
