# Ancestor Book Export — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add an "Ancestor Book" PDF export — a new "Stamtavla" tab in ReportsView that generates a self-contained PDF with a static circle chart, full ahnentafel ancestor list, and one personal summary per ancestor, all linked via internal PDF anchor links.

**Architecture:** `fetchAllAncestors()` (BFS, unlimited depth) in `chartData.ts` → new `AncestorBookReport.vue` that calls it, reuses `computeCircleLayout` for static SVG, and follows the `IndividualSummary.vue` pattern for person data. `ReportsView.vue` gets a new tab. All links are `href="#person-{id}"` fragments that Chromium preserves in `printToPDF()`.

**Tech Stack:** Vue 3 Composition API, existing `computeCircleLayout` / `fetchPedigreeTree` / `fetchPersonNode`, same `window.api.*` IPC calls as IndividualSummary.vue. No new IPC channels, no schema changes.
...
| File | Action |
|------|--------|
| `src/renderer/utils/chartData.ts` | Add `fetchAllAncestors()` at end of file |
| `src/renderer/i18n/sv.ts` | Add keys inside `reports:` block |
| `src/renderer/i18n/en.ts` | Add keys inside `reports:` block |
| `src/renderer/components/reports/AncestorBookReport.vue` | Create — full new component |
| `src/renderer/views/ReportsView.vue` | Add `ancestorBook` tab |

---

## Task 1: `fetchAllAncestors` in `chartData.ts`

**Files:**
- Modify: `src/renderer/utils/chartData.ts` (append after `fetchTimelineEntries`)

The spec says no unit tests for this function (it's a BFS loop over existing well-tested API calls).

- [ ] **Step 1: Add `fetchAllAncestors` to `chartData.ts`**

Append the following after the closing brace of `fetchTimelineEntries`:
...
Expected: no errors related to `fetchAllAncestors`.
...
Expected: all existing tests pass (there are no new tests for this function).

- [ ] **Step 4: Commit**

```bash
git add -A
git commit -m "feat: add fetchAllAncestors — BFS ancestor tree without generation cap"
```
...
**Files:**
- Modify: `src/renderer/i18n/sv.ts` — inside the `reports:` block (after `exportPdf: 'Exportera PDF'`)
- Modify: `src/renderer/i18n/en.ts` — inside the `reports:` block (after `exportPdf: 'Export PDF'`)

- [ ] **Step 1: Add Swedish strings to `sv.ts`**

Find the line `exportPdf: 'Exportera PDF',` inside `reports:` and add the following after it (before the closing `},`):
...
Find the line `exportPdf: 'Export PDF',` inside `reports:` and add the following after it:
...
```bash
git add -A
git commit -m "feat: add i18n strings for ancestor book report"
```

---

## Task 3: `AncestorBookReport.vue`

**Files:**
- Create: `src/renderer/components/reports/AncestorBookReport.vue`

This is a complete single-file component. Create the file with the following full content:

- [ ] **Step 1: Create `AncestorBookReport.vue`**
...
Expected: no errors. The most likely error is a missing import — double-check that `fetchAllAncestors` is imported from `chartData`.
...
Expected: all tests pass (no new tests for this component).

- [ ] **Step 4: Commit**

```bash
git add -A
git commit -m "feat: add AncestorBookReport component — static SVG + ahnentafel list + person summaries"
```

---

## Task 4: Add "Stamtavla" tab to `ReportsView.vue`

**Files:**
- Modify: `src/renderer/views/ReportsView.vue`

- [ ] **Step 1: Import `AncestorBookReport`**
...
- [ ] **Step 2: Add `ancestorBook` to the `activeTab` type and `tabs` computed**

Change the `activeTab` ref from:
```typescript
...
- [ ] **Step 3: Add the `ancestorBook` ref and tab content**

After `const individualPersonId = ref<string | null>(null);`, add:

```typescript
const ancestorBookPersonId = ref<string | null>(null);
```

In the `<template>`, after the closing `</div>` of the Individual Summary tab block (the `v-if="activeTab === 'individual'"` div), add:
...
Navigate to Reports → Stamtavla tab. Select a person. Verify:
- Loading state appears while data fetches
- Circle chart SVG renders with branch colors
- Ahnentafel list appears with generation headings and linked names
- Person summary sections appear for each ancestor
- Clicking a linked name scrolls to the correct `#person-{id}` anchor
- "Exportera PDF" button is enabled when a person is selected

- [ ] **Step 7: Commit**

```bash
git add -A
git commit -m "feat: add Stamtavla tab to ReportsView — ancestor book PDF export"
```
...
All four tasks complete. The ancestor book export is available in ReportsView → Stamtavla tab. Update `.claude/PLAN.md` to mark all checkboxes in the Ancestor Book Export roadmap section as done and move the plan files to archive.
