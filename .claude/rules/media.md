...
- A **relative path** like `family-media/photo.jpg` — resolved against `dbDir` at read time
- `null` — record exists but no file (citation-only)

`file_ref` should NEVER be an absolute path in a settled database. Absolute paths only appear transiently mid-import; they are normalised to relative refs by `consolidateMediaFolder` before the import IPC handler returns.
...
`consolidateMediaFolder` is idempotent and a near-no-op when refs are already relative — safe to call after every import.

## What NOT to do

- Do NOT hardcode folder names like `'genney-media'`, `'media'`, or `'photos'`. Always use `getMediaDir(dbPath)` / `getMediaFolderName(dbPath)`.
- Do NOT leave absolute `file_ref` values in the DB after an import handler returns. Consolidate must run.
- Do NOT skip consolidate for "small" or "trusted" imports — it's O(n_media) and idempotent.
- Do NOT compute or persist resolved absolute paths from a `file_ref` (those are derived; render-time only).
