...
- **Never recompute an expensive whole-DB aggregate on every `onDataChanged`.** A sidebar badge count that re-runs a full-DB scan per edit (`checks.runAll()`, `duplicates.count()`) serializes on the single SQLite connection and — on a tens-of-thousands-row DB — starves every visible-view reload, so a one-field edit takes *seconds* to appear in the chart/list/panel. The badge must derive from the **owning view's already-computed result** (a Pinia store the view writes — e.g. `useQualityStore`, `useDuplicateCountStore` fed by `usePagedList.total`), not an independent rescan. Cheap per-edit reloads (a paged `listPage`, a single-entity `useEntityData`) are fine; full-table scans on the per-edit path are the anti-pattern. The badge may lag until the owning page is next visited — acceptable for a hint. Regressed and fixed in `docs/plans/archive/2026-06-17-instant-updates-on-large-databases.md`; guarded by `tests/components/App.test.ts` (asserts no `runAll`/`duplicates.count` on mount).

Mechanism: `src/renderer/tauri-window-api.ts` wraps every mutating `window.api` binding in `mutating()` which fans out to `dataChangedListeners` (and cross-window via the Tauri event bus). Composables register/unregister against this source of truth.
