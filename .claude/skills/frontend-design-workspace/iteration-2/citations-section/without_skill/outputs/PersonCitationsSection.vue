    ...
    <table v-else class="data-table">
      ...
      <tbody>
        <tr
          ...
          :key="citation.id"
          class="clickable-row"
          @click="goToSource(citation.source_id)"
        >
          <td>{{ citation.source_title || '—' }}</td>
          <td class="td-page">{{ citation.page || '—' }}</td>
          <td class="td-shrink">
            <span v-if="citation.confidence != null" class="confidence-badge" :class="'confidence-' + citation.confidence">
...
<style scoped>
.th-page, .td-page { max-width: 120px; }
...
.confidence-1 { background: #fff7ed; color: #92400e; }
.confidence-2 { background: #fefce8; color: #854d0e; }
