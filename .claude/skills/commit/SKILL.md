...
- **NEVER amend** unless explicitly asked — always create a new commit. Two independent hazards, both from the same cause:
  - **The index moved.** `--amend` re-reads the index, so a file staged since the original commit rides along silently.
  - **HEAD moved.** `--amend` rewrites whatever HEAD is *now*, not the commit you made a moment ago. **`.claude/hooks/git-safety.sh` blocks an amend when HEAD already exists on a remote** — such a commit is either someone else's or shared history. Amending a local-only commit stays allowed.

  **The incident (2026-08-23).** The agent committed, then ran `--amend -s` to add a sign-off it had forgotten. Between the two commands another session committed *and pushed*; `--amend` rewrote **their** commit instead. No damage — identical tree, never pushed, recovered with `git reset --hard origin/main` — but only by luck. Note the amend was not asked for: it was self-initiated housekeeping, which is exactly the shape this rule forbids. Small self-directed fixes are where the rule gets skipped.
...
- **Read-then-act gaps are the repo's dominant failure mode.** Every git incident here has the same shape — state is observed, then acted on, and the repo moved in between:

  | Observed | Acted | What moved |
  |---|---|---|
  | `git status` clean | `git add -A` | files appeared |
  | HEAD was my commit | `git commit --amend` | HEAD advanced and was pushed |
  | branch was `main` | committed | branch had been switched |

  Two of the three are now blocked mechanically. For anything else: re-read state in the *same* command as the action where possible, and never carry an observation across a tool call you did not have to make.
- **NEVER skip hooks** (no `--no-verify`)
- **DCO sign-off: `git commit -s`, on every commit that will reach a PR.** The repo runs the DCO probot check, which requires each commit to carry `Signed-off-by:` matching its author exactly. It is **advisory today** — `main` is not branch-protected, which is why unsigned commits have landed via direct push — but it *blocks nothing only until someone turns protection on*.

  **`format.signOff` does NOT do this.** Git's own docs: *"A boolean value which lets you enable the `--signoff` option of **format-patch** by default."* It has no effect on `git commit`. Setting it and assuming commits are signed produces unsigned commits and a failing check. Verified on 2026-08-23: config set to `true`, next commit had no trailer. If you want it automatic, that is a `prepare-commit-msg` hook, not a config — and note git deliberately makes it manual, since the trailer *"should be a conscious act"*.

  Prevalence, so nobody repeats the mistake of inferring a convention from a sample: **85 of 2871 commits on `main` carry the trailer (3 %)**, mostly dependabot. It is not yet the norm here. See `.claude/rules/evidence.md`.
